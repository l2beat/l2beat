...
import { getKeyFindings, type KeyFinding } from './narrative'
...
export function reviewToMarkdown(review: CompiledReview): string {
  ...
  if (funds.length === 0) {
    ...
    const totalFunds = funds.reduce(
      (sum, f) =>
        sum + (f.balances?.totalUsdValue ?? 0) + (f.positions?.totalUsdValue ?? 0),
      0,
    )
    lines.push(
        ...
        for (const fn of admin.functions) {
          const fundsStr =
            fn.directFundsUsd > 0
              ...
              : ''
          lines.push(
            `- \`${fn.contractName}.${fn.functionName}()\`${fundsStr}`,
          )
        }
      }
      lines.push('')
  ...
  lines.push('---')
  lines.push(`*Generated by DeFiScan on ${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}*`)
  lines.push('')
