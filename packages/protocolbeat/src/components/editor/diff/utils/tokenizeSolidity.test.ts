...
describe('tokenizeSolidity', () => {
  it('returns no tokens for empty source', () => {
    expect(tokenizeSolidity('')).toStrictEqual([])
  })
  ...
  it('emits identifier as a single structural token', () => {
    expect(tokenizeSolidity('hello')).toStrictEqual([structural('hello', 1, 1)])
  })

  it('emits each punctuation char as its own token', () => {
    expect(tokenizeSolidity('a&&b')).toStrictEqual([
      structural('a', 1, 1),
      structural('&', 1, 1),
      structural('&', 1, 1),
      structural('b', 1, 1),
  ...
  it('whitespace between tokens does not change the token stream', () => {
    ...
    expect(stripPositions(tight)).toStrictEqual(stripPositions(loose))
  })
  ...
  it('does not treat /* inside a string as a comment', () => {
    ...
    expect(tokens).toStrictEqual([structural('"/* nope */"', 1, 1)])
  })

  it('handles escaped quote inside a string', () => {
    const tokens = tokenizeSolidity('"a\\"b"')
    expect(tokens).toStrictEqual([structural('"a\\"b"', 1, 1)])
  })

  it('handles single-quoted strings', () => {
    const tokens = tokenizeSolidity("'hello'")
    expect(tokens).toStrictEqual([structural("'hello'", 1, 1)])
  })

  it('preserves whitespace inside strings verbatim', () => {
    const a = tokenizeSolidity('"a   b"')
    const b = tokenizeSolidity('"a b"')
    expect(a[0]?.content).toStrictEqual('"a   b"')
    expect(b[0]?.content).toStrictEqual('"a b"')
    expect(a[0]?.content === b[0]?.content).toStrictEqual(false)
  })
  ...
  it('unterminated block comment extends to EOF', () => {
    ...
    expect(tokens[0]?.type).toStrictEqual('comment')
    expect(tokens[0]?.startLine).toStrictEqual(1)
    expect(tokens[0]?.endLine).toStrictEqual(2)
  })

  it('unterminated string extends to EOF', () => {
    const tokens = tokenizeSolidity('"never ends')
    expect(tokens.length).toStrictEqual(1)
    expect(tokens[0]?.type).toStrictEqual('structural')
    expect(tokens[0]?.content).toStrictEqual('"never ends')
  })

  it('splits numeric literal with dot into separate tokens', () => {
    expect(tokenizeSolidity('1.5')).toStrictEqual([
      structural('1', 1, 1),
      structural('.', 1, 1),
      structural('5', 1, 1),
    ])
  ...
  it('hex numeric literal is a single identifier token', () => {
    expect(tokenizeSolidity('0xFF')).toStrictEqual([structural('0xFF', 1, 1)])
  })
  ...
  it('member access "a.b" yields three tokens regardless of spacing', () => {
    ...
    expect(tight).toStrictEqual([
      structural('a', 1, 1),
      structural('.', 1, 1),
      structural('b', 1, 1),
    ])
    expect(loose).toStrictEqual(tight)
  })
})
