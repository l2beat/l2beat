...
describe('removeJSONTrailingCommas', () => {
  it('does nothing when there are no trailing commas', () => {
    const input = '{"a":1,"b":2}'
    expect(removeJSONTrailingCommas(input)).toStrictEqual(input)
  })

  it('removes a trailing comma in an object', () => {
    const input = '{"a":1,"b":2,}'
    expect(removeJSONTrailingCommas(input)).toStrictEqual('{"a":1,"b":2}')
  })

  it('removes a trailing comma in an array', () => {
    const input = '[1,2,]'
    expect(removeJSONTrailingCommas(input)).toStrictEqual('[1,2]')
  })

  it('removes nested trailing commas', () => {
    const input = '{"a":[1,2,],}'
    expect(removeJSONTrailingCommas(input)).toStrictEqual('{"a":[1,2]}')
  })

  it('preserves commas inside string values and keys', () => {
    const input = '{"k,e,y":"v,a,l,u,e",}'
    expect(removeJSONTrailingCommas(input)).toStrictEqual(
      '{"k,e,y":"v,a,l,u,e"}',
    )
  })
