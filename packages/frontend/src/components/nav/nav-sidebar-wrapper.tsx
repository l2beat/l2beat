...
export const NavSideBarWrapper = ({
  ...
  return (
    <>
      {open && (
        <div className="fixed left-0 top-0 z-999 size-full bg-black/25 max-sm:hidden" />
      )}
      <div
        ref={ref}
        className={cn(
          ...
          sharedSizeClasses,
          open && 'translate-x-0 sm:translate-x-[calc(100%-320px)]',
          resizing && 'transition-none',
        )}
      >
        <div
          className={cn(
            'flex flex-col gap-6 overflow-y-auto overflow-x-clip bg-surface-primary px-3.5 pt-4 transition-all duration-300 ease-out sm:w-80 sm:bg-background lg:fixed lg:px-5 lg:pt-[1.125rem]',
            sharedSizeClasses,
            resizing && 'transition-none',
          )}
        >
