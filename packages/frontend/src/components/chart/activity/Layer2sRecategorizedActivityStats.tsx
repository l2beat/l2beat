...
} from '~/pages/layer2s/activity/components/ActivityMetricContext'
...
export function Layer2sRecategorizedActivityStats({ entries }: Props) {
  const trpc = useTRPC()
  ...
  const scalingFactorFooter =
    scalingFactor !== undefined ? (
      <div className="flex items-center gap-1 pt-1 text-secondary">
        ...
        <Layer2sFactorTooltip metric={metric} />
      </div>
    ) : undefined
...
function Layer2sFactorTooltip({ metric }: { metric: ActivityMetric }) {
  return (
    <Tooltip>
      <TooltipTrigger>
