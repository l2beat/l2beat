...
export function InteropTransfersTable({
  ...
  const fromKey = apiSelection.from.join(',')
  const toKey = apiSelection.to.join(',')
  useEffect(() => {
    setSelectedFrom(fromKey === '' ? [] : fromKey.split(','))
    setSelectedTo(toKey === '' ? [] : toKey.split(','))
