...
export function TvlSummary(props: TvlSummaryProps) {
  ...
  return props.detailedTvlEnabled && !props.showTvlBreakdown ? (
  ...
  ) : props.detailedTvlEnabled && props.showTvlBreakdown ? (
    <div className="flex w-full flex-col items-center bg-gray-100 p-4 dark:bg-zinc-800 md:w-[30%] md:rounded-lg md:px-6 md:py-5">
      <div className="md:justify-normal mb-0 flex w-full items-end justify-between md:mb-4  md:gap-2">
        <span className="text-lg font-bold text-gray-500 dark:text-gray-600 md:text-xs md:font-normal">
          TVL
        </span>
        {props.stats ? (
          <div className="flex items-center gap-2 md:gap-1">
      ...
      {props.stats ? (
        <>
          <div className="mb-3 h-1/2 w-full">
            {tvlStats.map((s, i) => (
              <div key={i} className="flex w-full items-end justify-between">
                <span className="text-xs text-gray-500 dark:text-gray-600">
                  {s.label}
