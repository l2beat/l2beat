...
const getCachedDaProjectTvl = cache(
  async <T extends string>(bridgeUsedIn: Record<T, ProjectId[]>) => {
    const projectIds = [...new Set(Object.values(bridgeUsedIn).flat())] as ProjectId[];

    const allValues = projectIds.length === 0 ? [] : await db.value.getLatestValues(projectIds);

    return (Object.keys(bridgeUsedIn) as T[]).reduce((acc, curr) => {
      ...
      const values = allValues.filter((value) => projectIds.includes(value.projectId));
...
const getMockDaProjectTvl = <T extends string>(
  ...
  return Object.fromEntries(
    (Object.entries(bridgeUsedIn) as [keyof typeof bridgeUsedIn, ProjectId[]][]).map(
      ([bridge, projectIds]) => [bridge, projectIds.length * 1000]
    )
  );
};
