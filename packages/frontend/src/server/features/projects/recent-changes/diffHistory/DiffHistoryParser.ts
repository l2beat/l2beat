...
export class DiffHistoryParser {
  parse(content: string): DiffHistoryEntry[] {
    return this.parseWithMap(content, (entry) => entry)
  }

  parseWithMap<T>(
    content: string,
    map: (entry: DiffHistoryEntry) => T | null,
    limit = Number.POSITIVE_INFINITY,
  ): T[] {
    const results: T[] = []
    for (const entry of this.iterateEntries(content)) {
  ...
  private *iterateEntries(content: string): Generator<DiffHistoryEntry> {
    ...
    while (i < lines.length) {
      ...
      if (headerMatch) {
        ...
        yield this.parseEntry(date, pendingHash, lines.slice(i + 1, j))
        pendingHash = null
        i = j
        continue
      }
