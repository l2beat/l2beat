...
- **Ossification (0–100):** the interpolated percentile of the perimeter's age
  ...
  exploits whose exploited code was younger. The curve is a versioned dataset
  release (`ossificationCurve.ts`, currently v2026-08, n=243, ages verified
  onchain; regenerated by `scripts/ossification-incidents-curve.ts`). The
  public dataset lives in the standalone `ossification-dataset` repo
  (per-protocol incident files, deterministic curve rebuild, onchain
  re-verification script); `build-curve.mjs --check` there must match this
  file's knots. Scores change only at deliberate dataset releases, never
  silently.
