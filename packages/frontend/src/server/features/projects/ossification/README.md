...
- The runtime never parses `diffHistory.md`. It consumes `changelog.json` — a
  committed, deterministic projection of the markdown's watched changes
  (mechanical facts only: entry id, run timestamp, per-contract field diffs
  with raw values). l2b regenerates the file on every discovery run for
  projects that have one (`updateDiffHistory.ts`);
  `scripts/ossification-build-changelog.ts` seeds new opt-ins and its
  `--check` mode verifies the committed projection in CI. Update ids are
  stable (entry date + chain point + per-file ordinal), so a human editing an
  update's description never orphans `criticalEvents.updateId` references.
- State-change history is re-evaluated against current judgment: the runtime
  reads the changelog field by field and counts a state event iff
