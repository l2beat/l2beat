...
export async function getTvlProjectStats(project: Layer2 | Layer3 | Bridge) {
  if (env.MOCK) {
    return getMockTvlProjectStatsData()
  }
  return getTvlProjectStatsData(project)
}

type TvlProjectStats = Awaited<ReturnType<typeof getTvlProjectStatsData>>
async function getTvlProjectStatsData(project: Layer2 | Layer3 | Bridge) {
...
function getMockTvlProjectStatsData(): TvlProjectStats {
  return {
    tokenBreakdown: {
      total: 100,
      ether: 80,
