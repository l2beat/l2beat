...
import { getL2DaSolutions } from './getL2DaSolutions'
import type { L2Rosette } from './getL2RosetteValues'
import { getL2Rosette } from './getL2RosetteValues'

export interface ProjectL2Entry {
  id: ProjectId
  type: 'layer3' | 'layer2'
  name: string
...
export async function getL2ProjectEntry(
  project: Project<
    | 'display'
    | 'statuses'
    | 'scalingInfo'
  ...
  const daSolutions = await getL2DaSolutions(project)
  const [
  ...
  const hostChainRisksSummary =
    hostChain &&
    getL2RiskSummarySection(hostChain, hostChainVerificationWarnings, [])
  ...
  const riskSummary = getL2RiskSummarySection(
    project,
