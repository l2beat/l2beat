...
function deploymentsToChainInfo(
  ...
  for (const deployment of deployments) {
    ...
    const layer2sProject = projectsWithChain.find(
      (c) => c.chainConfig.name === deployment.chain,
    )
    if (layer2sProject) {
      map.set(deployment.chain, {
        name: layer2sProject.name,
        iconUrl: manifest.getUrl(`/icons/${layer2sProject.slug}.png`),
        explorerUrl: layer2sProject.chainConfig.explorerUrl,
      })
    }
  }
  return map
