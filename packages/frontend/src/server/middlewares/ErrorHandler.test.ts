...
describe(clientErrorStatus.name, () => {
  ...
  it('ignores an error without a status', () => {
    expect(clientErrorStatus(new Error('database is down'))).toStrictEqual(
      undefined,
    )
  })
})
