...
/**
 * Cloudflare caches HTML only when the origin sends Cache-Control. Browsers
 * keep revalidating (max-age=0) so a deploy is visible immediately once the
 * edge is purged. The edge serves the old copy while one request refreshes it
 * and keeps the site up if the origin returns 5xx.
 */
