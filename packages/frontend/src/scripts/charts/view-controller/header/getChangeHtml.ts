export function getChangeHtml(value: string) {
  ...
  const arrowDown = `
  <svg width="10" height="5" viewBox="0 0 10 5" role="img" aria-label="Arrow down icon" alt-text="-" class="inline-block stroke-black dark:stroke-white rotate-180 fill-red-300 absolute top-1/2 translate-y-[-50%]">
  ...
  `
  const arrowUp = `
  <svg width="10" height="5" viewBox="0 0 10 5" role="img" aria-label="Arrow up icon" alt-text="+" class="inline-block stroke-black dark:stroke-white fill-green-300 dark:fill-green-450 absolute top-1/2 translate-y-[-50%]">
