...
describe('calculatePercentageChange', () => {
  ...
  it('returns 0 when now is less than 0.01', () => {
    expect(calculatePercentageChange(0.005, 100)).toBe(0)
  })

  it('calculates the correct percentage change', () => {
    expect(calculatePercentageChange(150, 100)).toBe(0.5)
  })

  it('returns 0 for invalid change values', () => {
    expect(calculatePercentageChange(Number.POSITIVE_INFINITY, 100)).toBe(0)
...
describe('formatPercent', () => {
  it('formats values greater than or equal to 1000 as ">1K%"', () => {
    expect(formatPercent(10)).toBe('>1K%')
  })
