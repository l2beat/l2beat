...
describe('dates', () => {
  describe(formatTimestampToDateWithHour.name, () => {
    it('12:00', () => {
      ...
      expect(formatTimestampToDateWithHour(date)).toBe(
  ...
  describe(formatTimestamp.name, () => {
    describe('without options', () => {
      it('returns date with short month name and without time', () => {
        ...
        expect(formatTimestamp(date1)).toBe('2021 Apr 21')
        const date2 = UnixTime.fromDate(new Date('1968-12-31T11:30:00Z'))
        expect(formatTimestamp(date2)).toBe('1968 Dec 31')
        const date3 = UnixTime.fromDate(new Date('2030-01-01T05:00:00Z'))
        expect(formatTimestamp(date3)).toBe('2030 Jan 01')
      })
    })
    ...
    describe('with options', () => {
      describe('mode', () => {
        describe('date', () => {
          it('returns date with short month name and without time', () => {
            const date1 = UnixTime.fromDate(new Date('2021-04-21T12:00:00Z'))
            expect(formatTimestamp(date1, { mode: 'date' })).toBe('2021 Apr 21')
            const date2 = UnixTime.fromDate(new Date('1968-12-31T11:30:00Z'))
            expect(formatTimestamp(date2, { mode: 'date' })).toBe('1968 Dec 31')
            const date3 = UnixTime.fromDate(new Date('2030-01-01T05:00:00Z'))
            expect(formatTimestamp(date3, { mode: 'date' })).toBe('2030 Jan 01')
          })
        })
        ...
        describe('datetime', () => {
          it('returns date with short month name and time', () => {
            const date1 = UnixTime.fromDate(new Date('2021-04-21T12:00:00Z'))
            expect(formatTimestamp(date1, { mode: 'datetime' })).toBe(
              '2021 Apr 21, 12:00 UTC',
            )
            const date2 = UnixTime.fromDate(new Date('1968-12-31T11:30:00Z'))
            expect(formatTimestamp(date2, { mode: 'datetime' })).toBe(
              '1968 Dec 31, 11:30 UTC',
            )
            const date3 = UnixTime.fromDate(new Date('2030-01-01T05:00:00Z'))
            expect(formatTimestamp(date3, { mode: 'datetime' })).toBe(
        ...
        describe('time', () => {
          it('returns time', () => {
            const date1 = UnixTime.fromDate(new Date('2021-04-21T12:00:00Z'))
            expect(formatTimestamp(date1, { mode: 'time' })).toBe('12:00 UTC')
            const date2 = UnixTime.fromDate(new Date('1968-12-31T11:30:00Z'))
            expect(formatTimestamp(date2, { mode: 'time' })).toBe('11:30 UTC')
            const date3 = UnixTime.fromDate(new Date('2030-01-01T05:00:00Z'))
            expect(formatTimestamp(date3, { mode: 'time' })).toBe('05:00 UTC')
          })
        })
      })
      ...
      describe('longMonthName', () => {
        it('returns date with long month name and without time', () => {
          const date1 = UnixTime.fromDate(new Date('2021-04-21T12:00:00Z'))
          expect(formatTimestamp(date1, { longMonthName: true })).toBe(
            '2021 April 21',
          )
          const date2 = UnixTime.fromDate(new Date('1968-12-31T11:30:00Z'))
          expect(formatTimestamp(date2, { longMonthName: true })).toBe(
            '1968 December 31',
          )
          const date3 = UnixTime.fromDate(new Date('2030-01-01T05:00:00Z'))
          expect(formatTimestamp(date3, { longMonthName: true })).toBe(
            '2030 January 01',
          )
        })
      })
  ...
  describe(getNextDateForDayOfWeek.name, () => {
    it('returns next date for given day of the week', () => {
      ...
      expect(nextThursday.toISOString()).toBe('2021-04-22T00:00:00.000Z')
    })
  })
})
