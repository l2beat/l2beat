...
export const PRIVACY_LEAK_VERDICT_LABEL: Record<PrivacyLeakVerdict, string> = {
  private: 'private',
  atRisk: 'at risk',
  leaked: 'leaked',
...
export const PRIVACY_LEAK_VERDICT_CLASS_NAME: Record<
  PrivacyLeakVerdict,
  string
> = {
  private: 'text-[#17452A] bg-[#C8F2D7] border-[#4FB875]',
  atRisk: 'text-[#5C3B00] bg-[#FFE8A3] border-[#D9A31A]',
  leaked: 'text-[#5D1111] bg-[#FFC9C9] border-[#E06565]',
...
export function getLeakVerdict(leak: PrivacyLeak): PrivacyLeakVerdict {
  return typeof leak === 'string' ? leak : leak.verdict
}

export function getLeakNote(leak: PrivacyLeak): string | undefined {
