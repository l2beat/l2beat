...
describe(detectImageMimeType.name, () => {
  it('detects a png', () => {
    ...
    expect(result).toStrictEqual('image/png')
  })
  ...
  it('rejects a format satori cannot decode', () => {
    ...
    expect(detectImageMimeType(webp)).toStrictEqual(undefined)
  })

  it('rejects a non-image response body', () => {
    const errorPage = Buffer.from('<!DOCTYPE html><title>404</title>')

    expect(detectImageMimeType(errorPage)).toStrictEqual(undefined)
  })
  ...
  it('rejects an empty body', () => {
    expect(detectImageMimeType(Buffer.alloc(0))).toStrictEqual(undefined)
  })
})
