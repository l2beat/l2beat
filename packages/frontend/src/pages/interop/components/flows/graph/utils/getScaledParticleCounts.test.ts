...
describe(getScaledParticleCounts.name, () => {
  it('returns the original counts at $50 when both caps are satisfied', () => {
    ...
    expect(result.counts).toStrictEqual([12, 7, 0.5])
    expect(result.dollarsPerParticle).toStrictEqual(50)
  })
  ...
  it('steps up to the next allowed value when per-flow cap is exceeded', () => {
    ...
    expect(result.counts).toStrictEqual([50, 35, 5])
    expect(result.dollarsPerParticle).toStrictEqual(100)
  })
  ...
  it('steps up to the next allowed value when global cap is exceeded', () => {
    ...
    expect(result.counts).toStrictEqual(Array.from({ length: 20 }, () => 30))
    expect(result.dollarsPerParticle).toStrictEqual(100)
  })
  ...
  it('continues in $50 multiples beyond the last fixed option', () => {
    ...
    expect(Math.max(...result.counts) <= 60).toStrictEqual(true)
    expect(result.counts.reduce((s, c) => s + c, 0) <= 700).toStrictEqual(true)
    expect(result.dollarsPerParticle).toStrictEqual(200)
  })
  ...
  it('scales a low base up through the fixed options', () => {
    ...
    expect(result.dollarsPerParticle).toStrictEqual(5)
    expect(result.counts).toStrictEqual([20, 14, 2])
  })

  it('keeps a sub-dollar base when caps are satisfied', () => {
    const result = getScaledParticleCounts([3, 1], 0.1)
    expect(result.counts).toStrictEqual([3, 1])
    expect(result.dollarsPerParticle).toStrictEqual(0.1)
  })
  ...
  it('scales a sub-dollar base to the next sub-dollar option', () => {
    ...
    expect(result.dollarsPerParticle).toStrictEqual(0.5)
    expect(result.counts).toStrictEqual([40])
  })
  ...
  it('returns quickly for very large values without iterating', () => {
    ...
    expect(elapsed < 50).toStrictEqual(true)
    expect(Math.max(...result.counts) <= 60).toStrictEqual(true)
    expect(result.counts.reduce((s, c) => s + c, 0) <= 700).toStrictEqual(true)
  })
  ...
  it('handles a single flow exactly at the per-flow cap', () => {
    ...
    expect(result.counts).toStrictEqual([60])
    expect(result.dollarsPerParticle).toStrictEqual(50)
  })
  ...
  it('handles totals exactly at the global cap', () => {
    ...
    expect(result.dollarsPerParticle).toStrictEqual(50)
    expect(result.counts.reduce((s, c) => s + c, 0)).toStrictEqual(700)
  })
})
