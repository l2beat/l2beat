...
import { getLayer2sCostsEntries } from '~/server/features/layer2s/costs/getLayer2sCostsEntries'
...
export async function getLayer2sCostsData(
...
async function getCachedData(cache: InMemoryCache, tab: 'rollups' | 'others') {
  ...
  const [entries, queryState] = await Promise.all([
    getLayer2sCostsEntries(helpers),
    cache.get(
      {
