...
import { getLayer2sSummaryValidiumAndOptimiumsColumns } from './columns'

interface Props {
  entries: Layer2sSummaryEntry[]
}

export function Layer2sSummaryValidiumsAndOptimiumsTable({ entries }: Props) {
  const trpc = useTRPC()
  ...
  const columns = useMemo(
    () =>
      getLayer2sSummaryValidiumAndOptimiumsColumns({ isTvsLoading: isLoading }),
    [isLoading],
  )
