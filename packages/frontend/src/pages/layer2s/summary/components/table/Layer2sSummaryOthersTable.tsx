...
import { getLayer2sSummaryOthersColumns } from './columns'

interface Props {
  entries: Layer2sSummaryEntry[]
}

export function Layer2sSummaryOthersTable({ entries }: Props) {
  const trpc = useTRPC()
  ...
  const columns = useMemo(
    () => getLayer2sSummaryOthersColumns({ isTvsLoading: isLoading }),
    [isLoading],
  )
