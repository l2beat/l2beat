...
description: "Perps DEXs can let users keep property rights to their collateral and make execution and position management verifiable. We compare Hyperliquid and Lighter across property rights, order fairness, and position fairness."
publishedOn: "2026-07-02"
authorId: l2beat_research
tag: Research
---

A perpetuals DEX differs from CEXs like Binance or Bybit in two ways. First, custody: it is possible for a perps DEX to let users maintain property rights to their collateral, rather than giving it up for an IOU from the exchange. Second, verifiability: both the execution of the orders and the management of the position can be transparent and verifiable.

Perps in particular (and other leveraged products) require the exchange engine to be able to actively manage users positions for liquidation. To do so, perps DEXs employ position management mechanisms - such as the auto deleveraging (ADL) algorithm - whose verifiability is crucial, as they give power to the exchange to close and haircut positions while putting exchange solvency directly against user profits.

The comparison of leading perps DEXs, Hyperliquid and Lighter, can be organised into three categories:
...
During normal operations, Lighter validity proof are periodically verified on Ethereum, meaning the circuits can enforce that:
...
| Activity / Risk | Lighter | Hyperliquid |
| --- | --- | --- |
| Passive USDC holding | Permissionless exit, subject to Stage 0 upgrade risk | No permissionless exit path, custody rests on validator consensus |
| Resting limit orders | Operator has order flow discretion | Operator has order flow discretion |
| Order matching | Operator sees full quote ladder, matching follows proven price-time priority | Operator sees full quote ladder, matching unproven |
| Active position with TP/SL | Operator has order management discretion, proof enforces floor conditions | Operator has order management discretion, no enforced floor |
| Leveraged positions with liquidation risk | Operators has mark-price discretion, validity proof enforce floor conditions | Operator has mark price discretion, no enforced floor, governance can re-settle |
| Enforcement basis | Public circuit + onchain validity proof, rules are auditable and cryptographically enforced | Closed-source execution, no validity proof - rules rest on validator consensus, not independently verifiable |
