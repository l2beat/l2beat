...
NO_BUILTIN_SIMULATION_SIMPLE_BOOTLOADER_COMPILED_PATH = os.path.join(
    os.path.dirname(__file__), "simple_bootloader", "no_builtin_simulation_simple_bootloader_compiled.json"
)
...
def get_no_builtin_simulation_simple_bootloader_program() -> Program:
    with open(NO_BUILTIN_SIMULATION_SIMPLE_BOOTLOADER_COMPILED_PATH, "r") as file:
        return Program.Schema().loads(file.read())
...
def compute_program_hash_chain(
    ...
    encode_blake2s_input: bool = True,
    little_endian_for_blake2s: bool = False,
):
    """
...
def get_blake_program_hash(program: Program) -> int:
    return compute_program_hash_chain(program=program, program_hash_function=HashFunction.BLAKE)


def calculate_supported_simple_bootloader_hash_list() -> List[int]:
    simple_bootloader_program: Program = get_simple_bootloader_program()
    no_builtin_simulation_simple_bootloader_program: Program = get_no_builtin_simulation_simple_bootloader_program()
    return [
        poseidon_hash_many(array=simple_bootloader_program.data),
        calculate_blake2s_hash_from_felt252s(
            data=simple_bootloader_program.data,
            encode=True,
            prime=simple_bootloader_program.prime,
            little_endian=False,
        ),
        poseidon_hash_many(array=no_builtin_simulation_simple_bootloader_program.data),
        calculate_blake2s_hash_from_felt252s(
            data=no_builtin_simulation_simple_bootloader_program.data,
            encode=True,
            prime=no_builtin_simulation_simple_bootloader_program.prime,
            little_endian=False,
        ),
    ]
...
def main():
    ...
    with get_crypto_lib_context_manager(args.flavor):
        ...
        print("supported_simple_bootloader_hash_list:", supported_simple_bootloader_hash_list)
        supported_simple_bootloader_hash_list_hash = compute_hash_on_elements(data=supported_simple_bootloader_hash_list)
        print("supported_simple_bootloader_hash_list_hash:", supported_simple_bootloader_hash_list_hash)

        applicative_bootloader_program = get_applicative_bootloader_program()
        applicative_bootloader_program_hash = get_blake_program_hash(program=applicative_bootloader_program)
