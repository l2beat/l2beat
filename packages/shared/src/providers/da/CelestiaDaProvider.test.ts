...
describe(CelestiaDaProvider.name, () => {
  describe(CelestiaDaProvider.prototype.getBlobs.name, () => {
    it('fetches blobs from multiple blocks with different timestamps', async () => {
      const rpcClientMock = createMockRpcClient(false)
    ...
    it('fetches blobs when data is inside log field', async () => {
      const rpcClientMock = createMockRpcClient(true)
      const provider = new CelestiaDaProvider(rpcClientMock, 'celestia')
      const blobs = await provider.getBlobs(1, 2)
...
function mockData(blockNumber: number) {
  return [
    {
...
function createMockRpcClient(useLogs: boolean) {
      ...
      txs_results: [
        {
          log: useLogs
            ? [
                {
                  events: mockData(blockNumber),
                },
              ]
            : null,
          events: useLogs ? [] : mockData(blockNumber),
        },
      ],
    }),
