...
import { getTrpcClientUrl, getTrpcProxyTarget } from './trpcUrl'

describe('tRPC URL helpers', () => {
  describe(getTrpcClientUrl.name, () => {
    it('uses the local proxy path by default', () => {
      expect(getTrpcClientUrl(undefined)).toEqual('/trpc')
    })

    it('appends /trpc when given a server origin', () => {
      expect(getTrpcClientUrl('https://token-knowledge.example.com')).toEqual(
        'https://token-knowledge.example.com/trpc',
      )
      expect(
        getTrpcClientUrl('https://token-knowledge.example.com/api'),
      ).toEqual('https://token-knowledge.example.com/api/trpc')
    })

    it('does not duplicate /trpc when the endpoint already includes it', () => {
      expect(
        getTrpcClientUrl('https://token-knowledge.example.com/trpc'),
      ).toEqual('https://token-knowledge.example.com/trpc')
      expect(
        getTrpcClientUrl('https://token-knowledge.example.com/api/trpc/'),
      ).toEqual('https://token-knowledge.example.com/api/trpc')
    })
  })
