...
function prettyPrint(resolved: Map<string, AnalyzedData>) {
  const addressMap = new Map<string, string>()
  for (const [address, data] of resolved) {
    addressMap.set(address, data.analysis.name)
  }

  for (const [address, data] of resolved) {
    if (data.analysis.type === 'EOA') {
      continue
    }
    console.log(chalk.red(data.analysis.name), chalk.blue(address))
    for (const { name, value } of data.parameters) {
      prettyPrintValue(name, value, addressMap)
    }
  }
}
