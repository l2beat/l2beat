...
export async function analyzeMainBridge(
  ...
  const proxyContract = await addressAnalyzer.analyze(EthereumAddress(mainBridge.proxyAddress))
  const [implementationSlot, ownerSlot] = await Promise.all([
    provider.getStorageAt(mainBridge.proxyAddress, '0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc'),
    provider.getStorageAt(mainBridge.proxyAddress, '0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103'),
  ])
  const implementation = wordToAddress(implementationSlot)
  if (implementation != mainBridge.implementation) {
    console.log(`Warning: bridge implementation is not ${mainBridge.implementation}`)
  }
  const owner = wordToAddress(ownerSlot)
  const implementationContract = await addressAnalyzer.analyze(EthereumAddress(implementation))
