...
describeDatabase(DataAvailabilityRepository.name, (db) => {
  ...
  describe(DataAvailabilityRepository.prototype.upsertMany.name, () => {
    it('adds new rows', async () => {
      await repository.upsertMany([
        ...
        record('project-c', 'layer-b', START + UnixTime(1, 'days'), 400n),
      ])
  ...
  describe(DataAvailabilityRepository.prototype.getForDaLayerInTimeRange
    ...
    it('returns records for a DA layer within time range', async () => {
      await repository.upsertMany([
        record('project-a', 'layer-a', START, 100n),
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 200n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 300n),
        record('project-a', 'layer-a', START + UnixTime(3, 'days'), 400n),
      ])
  ...
  describe(DataAvailabilityRepository.prototype.getByProjectIdsAndTimeRange
    ...
    it('should return records for projects in given time range', async () => {
      await repository.upsertMany([
        record('project-a', 'layer-a', START, 100n),
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 1_000n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 10_000n),
        record('project-b', 'layer-a', START, 200n),
        record('project-b', 'layer-a', START + UnixTime(1, 'days'), 2_000n),
    ...
    it('allows to query for null from', async () => {
      await repository.upsertMany([
        record('project-a', 'layer-a', START, 100n),
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 1_000n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 1_000n),
      ])

      const results = await repository.getByProjectIdsAndTimeRange(
        ['project-a', 'project-b'],
        [null, START + UnixTime(1, 'days')],
      )
  ...
  describe(DataAvailabilityRepository.prototype.getByDaLayersAndTimeRange
    ...
    it('should return records for projects in given time range', async () => {
      await repository.upsertMany([
        ...
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 1_000n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 10_000n),
        record('project-b', 'layer-b', START, 200n),
        record('project-b', 'layer-b', START + UnixTime(1, 'days'), 2_000n),
        record('project-b', 'layer-b', START + UnixTime(2, 'days'), 2_000n),
        record('project-c', 'layer-a', START, 300n),
      ])
    ...
    it('allows to query for null from', async () => {
      await repository.upsertMany([
        record('project-a', 'layer-a', START - UnixTime(1, 'days'), 100n),
        record('project-a', 'layer-a', START, 100n),
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 1_000n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 1_000n),
        record('project-b', 'layer-b', START, 1_000n),
      ])
  ...
  describe(DataAvailabilityRepository.prototype
    ...
    it('returns summed PROJECTS (not the daLayer itself)', async () => {
      await repository.upsertMany([
        ...
        record('project-a', 'layer-a', START + UnixTime(1, 'days'), 1_000n),
        record('project-a', 'layer-a', START + UnixTime(2, 'days'), 1_000n),
        record('project-c', 'layer-b', START, 1_000n),
        record('project-b', 'layer-a', START, 10n),
        record('project-b', 'layer-a', START + UnixTime(1, 'days'), 2_000n),
        record('layer-a', 'layer-a', START + UnixTime(1, 'days'), 1_000_000n),
      ])
