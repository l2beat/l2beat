...
describeTokenDatabase(TokenIngestionQueueRepository.name, (db) => {
  ...
  describe(TokenIngestionQueueRepository.prototype.countPending.name, () => {
    it('counts only pending entries', async () => {
      ...
      expect(await repository.countPending()).toStrictEqual(1)
    })
  })
  ...
  describe(TokenIngestionQueueRepository.prototype.getByStates.name, () => {
    ...
    it('escapes NUL bytes in messages before writing them', async () => {
      ...
      expect(entries[0]?.message).toStrictEqual('RPC returned U\\0SDC')
    })
  })

  describe(TokenIngestionQueueRepository.prototype.getPage.name, () => {
    it('returns one page with the total count', async () => {
      await repository.enqueue({ chain: 'ethereum', address: '0x111' })
      await repository.enqueue({ chain: 'arbitrum', address: '0x222' })
      await repository.enqueue({ chain: 'base', address: '0x333' })
    ...
    it('filters by chains in both entries and total count', async () => {
      ...
      expect(page.totalCount).toStrictEqual(3)
      expect(page.entries.map((e) => e.address)).toStrictEqual([
        '0x333',
        '0x111',
        '0x222',
      ])
  ...
  describe(TokenIngestionQueueRepository.prototype.retry.name, () => {
    it('moves conflict and error entries back to pending', async () => {
      ...
      expect(await repository.retry(conflict)).toStrictEqual(1)
    ...
    it('does not touch already pending entries', async () => {
      ...
      expect(await repository.retry(address)).toStrictEqual(0)
    })
  })
  ...
  describe(TokenIngestionQueueRepository.prototype.approve.name, () => {
    it('moves staged entries to pending', async () => {
      ...
      expect(await repository.approve(address)).toStrictEqual(1)
    ...
    it('does not touch non-staged entries', async () => {
      ...
      expect(await repository.approve(address)).toStrictEqual(0)
    })
  })
  ...
  describe(TokenIngestionQueueRepository.prototype.remove.name, () => {
    it('deletes an entry', async () => {
      ...
      expect(await repository.remove(address)).toStrictEqual(1)
