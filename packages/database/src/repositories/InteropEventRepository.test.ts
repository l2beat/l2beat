...
describeDatabase(InteropEventRepository.name, (db) => {
  ...
  describe(InteropEventRepository.prototype.getUnmatched.name, () => {
    ...
    it('returns only unmatched and supported events', async () => {
      ...
      expect([...result.map((r) => r.eventId)].sort()).toEqual(
        ['event1', 'event4'].sort(),
      )
      result.forEach((event) => {
        expect(event.matched).toBe(false)
        expect(event.unsupported).toBe(false)
      })
    })
  })
  ...
  describe(
    ...
    () => {
      it('returns the oldest event by timestamp for plugins and chain', async () => {
        ...
        expect(result?.eventId).toBe('event4')
      })
  ...
  describe(InteropEventRepository.prototype.getByType.name, () => {
    ...
    it('filters by unsupported status when provided', async () => {
      ...
      expect(result).toHaveLength(1)
      expect(result[0]?.eventId).toBe('event3')
    })
  ...
  describe(InteropEventRepository.prototype.updateMatched.name, () => {
    ...
    it('updates matched status for specified event IDs', async () => {
      ...
      expect(event1?.matched).toBe(true)
      expect(event2?.matched).toBe(false)
      expect(event3?.matched).toBe(true)
    })
  ...
  describe(InteropEventRepository.prototype.updateUnsupported.name, () => {
    ...
    it('updates unsupported status for specified event IDs', async () => {
      ...
      expect(event1?.unsupported).toBe(true)
      expect(event2?.unsupported).toBe(false)
      expect(event3?.unsupported).toBe(true)
    })
    ...
    it('performs batch update when more than 2000 event IDs', async () => {
      ...
      const events = await repository.getAll()
      events.forEach((event) => {
        expect(event.unsupported).toBe(true)
      })
    })
  })
  ...
  describe(InteropEventRepository.prototype.updateDerivedFulfilled.name, () => {
    ...
    it('updates derivedFulfilled status for specified event IDs', async () => {
      ...
      expect(event1?.derivedFulfilled).toBe(true)
      expect(event2?.derivedFulfilled).toBe(false)
      expect(event3?.derivedFulfilled).toBe(true)
    })
  })
  ...
  describe(InteropEventRepository.prototype.deleteExpired.name, () => {
    it('deletes events that have expired', async () => {
      ...
      expect(remaining[0]?.eventId).toBe('event4')
    })
  ...
  describe(InteropEventRepository.prototype.deleteAllForPlugin.name, () => {
    it('deletes events for the specified plugin', async () => {
      ...
      const remaining = await repository.getAll()
      expect(remaining).toHaveLength(1)
      expect(remaining[0]?.eventId).toBe('event3')
    })
  ...
  describe('bigint handling in insertMany and getAll', () => {
    it('handles simple bigint value field', async () => {
      ...
      expect(result).toHaveLength(1)
      expect(result[0]?.args).toBe(123456789012345678901234567890n)
    })
