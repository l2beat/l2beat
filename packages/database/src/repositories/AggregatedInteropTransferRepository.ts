...
export class AggregatedInteropTransferRepository extends BaseRepository {
  ...
  async getGlobalStatsByTimestamp(
    timestamp: UnixTime,
  ): Promise<AggregatedInteropTransferGlobalStatsRecord | undefined> {
  ...
  async getRecentGlobalStats(
    limit: number,
    options?: {
      before?: UnixTime
    },
  ): Promise<AggregatedInteropTransferGlobalStatsRecord[]> {
    let query = this.db
      .selectFrom('AggregatedInteropTransfer')
      .select((eb) => [
        'timestamp',
    ...
    if (options?.before !== undefined) {
      query = query.where('timestamp', '<', UnixTime.toDate(options.before))
    }
  ...
  async getLatestTimestampBeforeForGroup(
    timestamp: UnixTime,
    id: string,
    bridgeType: InteropBridgeType,
    srcChain: string,
  ...
  async getStatsByTimestampForGroup(
    timestamp: UnixTime,
    id: string,
    bridgeType: InteropBridgeType,
    srcChain: string,
    dstChain: string,
  ): Promise<AggregatedInteropTransferGroupStatsRecord | undefined> {
    const row = await this.db
      .selectFrom('AggregatedInteropTransfer')
      .select((eb) => [
        'timestamp',
