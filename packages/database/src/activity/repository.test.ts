...
describeDatabase(ActivityRepository.name, (db) => {
  ...
  describe(ActivityRepository.prototype.upsertMany.name, () => {
    it('adds new rows', async () => {
      await repository.upsertMany([
        ...
        record('a', START + UnixTime(1, 'days'), 2, 2, 3, 4),
        record('a', START + UnixTime(2, 'days'), 4, 5, 5, 6),
      ])
    ...
    it('merges on conflict', async () => {
      await repository.upsertMany([
        record('a', START, 1, 2, 1, 2),
        record('a', START + UnixTime(1, 'days'), 2, 2, 4, 5),
      ])
  ...
  describe(ActivityRepository.prototype.deleteByProjectIdFrom.name, () => {
    it('should delete all rows after a given timestamp and projectId', async () => {
      await repository.upsertMany([
        record('a', START),
        record('a', START + UnixTime(1, 'days')),
        record('a', START + UnixTime(2, 'days')),
        record('a', START + UnixTime(3, 'days')),
      ])
  ...
  describe(ActivityRepository.prototype.getByProjectAndTimeRange.name, () => {
    it('should return all rows in a given time range for a project', async () => {
      ...
      const results = await repository.getByProjectAndTimeRange(
        ProjectId('a'),
        [START + UnixTime(1, 'days'), START + UnixTime(2, 'days')],
      )
  ...
  describe(ActivityRepository.prototype.getMaxCountsForProjects.name, () => {
    it('should return max UOPS count for each project', async () => {
      await repository.upsertMany([
        record('a', START, 1),
        record('a', START + UnixTime(1, 'days'), 3, 4),
        record('a', START + UnixTime(2, 'days'), 4, 7),
        record('b', START + UnixTime(1, 'days'), 6, 6),
        record('b', START + UnixTime(2, 'days'), 5, 8),
      ])
      ...
      expect(result).toEqual({
        [ProjectId('a')]: {
          uopsCount: 7,
          uopsTimestamp: START + UnixTime(2, 'days'),
          count: 4,
          countTimestamp: START + UnixTime(2, 'days'),
        },
        [ProjectId('b')]: {
          uopsCount: 8,
          uopsTimestamp: START + UnixTime(2, 'days'),
          count: 6,
          countTimestamp: START + UnixTime(1, 'days'),
        },
      })
    })
    ...
    it('should return max UOPS count when uops is null', async () => {
      await repository.upsertMany([
        // uopsCount is null
        record('a', START, 1),
        record('a', START + UnixTime(1, 'days'), 5),
        record('a', START + UnixTime(2, 'days'), 4),
        // normal count bigger then uopsCount
        record('b', START + UnixTime(1, 'days'), 2, 6),
        record('b', START + UnixTime(2, 'days'), 8),
        // uopsCount was null in the past
        record('c', START + UnixTime(1, 'days'), 4),
        record('c', START + UnixTime(2, 'days'), 5, 9),
      ])
      ...
      expect(result).toEqual({
        [ProjectId('a')]: {
          uopsCount: 5,
          uopsTimestamp: START + UnixTime(1, 'days'),
          count: 5,
          countTimestamp: START + UnixTime(1, 'days'),
        },
        [ProjectId('b')]: {
  ...
  describe(ActivityRepository.prototype
    ...
    it('should return summed count grouped by projectId for given time range', async () => {
      await repository.upsertMany([
        record('a', START, 1),
        record('a', START + UnixTime(1, 'days'), 3, 4),
        record('a', START + UnixTime(2, 'days'), 4, 6),
        // project without any uopsCount
        record('b', START + UnixTime(1, 'days'), 2),
        record('b', START + UnixTime(2, 'days'), 5),
      ])

      const result = await repository.getSummedUopsCountForProjectsAndTimeRange(
        [ProjectId('a'), ProjectId('b')],
        [START, START + UnixTime(2, 'days')],
      )
  ...
  describe(ActivityRepository.prototype.getByProjectIncludingDataPoint
    ...
    it('should return all rows including data point', async () => {
      await repository.upsertMany([
        ...
        record('a', START + UnixTime(1, 'days'), 1, 2, 11, 20),
        record('a', START + UnixTime(2, 'days'), 1, 1, 21, 30),
      ])
  ...
  describe(ActivityRepository.prototype.getDailyCounts.name, () => {
    it('should return correct response for single project', async () => {
      await repository.upsertMany([
        record('a', START, 1),
        record('a', START + UnixTime(1, 'days'), 1),
        record('a', START + UnixTime(2, 'days'), 1),
      ])
  ...
  describe(ActivityRepository.prototype.getProjectsAggregatedDailyCount
    ...
    it('should return correct response for single project', async () => {
      ...
      expect(result).toEqual(
        [
          record('a', START, 1),
          record('a', START + UnixTime(1, 'days'), 1),
        ].map((i) => omit(i, ['projectId', 'start', 'end', 'uopsCount'])),
      )
    })
    ...
    it('should return correct response for multiple projects', async () => {
      await repository.upsertMany([
        ...
        record('b', START, 3),
        record('a', START + UnixTime(1, 'days'), 1),
        record('b', START + UnixTime(1, 'days'), 3),
        record('c', START + UnixTime(1, 'days'), 4),
        record('c', START + UnixTime(2, 'days'), 2),
      ])
      ...
      expect(result).toEqual([
        ...
        { timestamp: START + UnixTime(1, 'days'), count: 8 },
        { timestamp: START + UnixTime(2, 'days'), count: 2 },
      ])
    })
  })
