...
describeDatabase(IndexerConfigurationRepository.name, (db) => {
  ...
  describe(IndexerConfigurationRepository.prototype.getByConfigurationIds
    .name, () => {
  ...
  it(IndexerConfigurationRepository.prototype.getByIndexerIds
    .name, async () => {
    const records = CONFIGURATIONS

    await repository.upsertMany(records)

    const result = await repository.getByIndexerIds(['indexer-1', 'indexer-2'])

    expect(result).toEqualUnsorted(records.slice(0, 3))
  })

  it(IndexerConfigurationRepository.prototype.getConfigurationsWithoutIndexerId
    .name, async () => {
    const records = CONFIGURATIONS
  ...
  it(IndexerConfigurationRepository.prototype.getIdsByIndexer
    .name, async () => {
    const records = CONFIGURATIONS
  ...
  it(IndexerConfigurationRepository.prototype.updateCurrentHeights
    .name, async () => {
    const records = [
  ...
  describe(IndexerConfigurationRepository.prototype.deleteConfigurations
    .name, () => {
