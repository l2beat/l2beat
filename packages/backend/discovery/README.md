...
- `descriptions: Record<address, object>` - (optional) key-value object, with addresses as keys. It will allow you to add descriptions to the contract and its fields (ex. `Bridge` contract with `field_a` field)
...
```
{
  ...
  descriptions: {
    ...
    "Rollup": {
      "description": "This is a Rollup contract",
    }
  }
}
```
