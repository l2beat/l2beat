    ...
    CrossDomainMessengerLegacySpacer1
{
    ...
    ///         value of this variable is the default value (0x00000000...0000) then no message is
    ///         currently being executed. Use the xDomainMessageSender getter which will throw an
    ///         error if this is the case.
    bytes32 internal xDomainMsgSender;
    ...
    event SentMessage(bytes32 indexed target, address sender, bytes message, uint256 messageNonce, uint256 gasLimit);
    ...
    function relayMessage(
        ...
        payable
    {
        uint256 valueLD = _value.ETHToLD();
        ...
        bytes32 versionedHash =
            Hashing.hashL2ToL1CrossDomainMessageV1(_nonce, _sender, _target, _value, _minGasLimit, _message);

        if (_isOtherMessenger()) {
            // These properties should always hold when the message is first submitted (as
            // opposed to being replayed).
            assert(msg.value == valueLD);
        ...
        bool success = SafeCall.call(_target, gasleft() - RELAY_RESERVED_GAS, valueLD, _message);
        xDomainMsgSender = Constants.DEFAULT_L2_SENDER;
    ...
    /// @custom:legacy
    function OTHER_MESSENGER() public view returns (bytes32) {
    ...
    function __CrossDomainMessenger_init(bytes32 _otherMessenger) internal onlyInitializing {
    ...
    function _sendMessage(bytes32 _to, uint64 _gasLimit, uint256 _value, bytes memory _data) internal virtual;
    ...
    /// @notice Encode L2 message to relay on L2.
    /// @param _target      Target contract or wallet address.
    /// @param _message     Message to trigger the target address with.
    /// @param _minGasLimit Minimum gas limit that the message can be executed with.
    function encodeRelayL2Message(
        uint256 _nonce,
        address _sender,
        bytes32 _target,
        uint256 _value,
        ...
        returns (bytes memory)
    {
        return Encoding.encodeL1ToL2CrossDomainMessage(_nonce, _sender, _target, _value, _minGasLimit, _message);
    }
}
