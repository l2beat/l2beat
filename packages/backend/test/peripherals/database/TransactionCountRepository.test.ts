...
describe(TransactionCountRepository.name, () => {
  ...
  describe(
    TransactionCountRepository.prototype.findLatestByProject.name,
    () => {
      it('returns undefined when no records', async () => {
        expect(
          await repository.findLatestByProject(PROJECT_A),
        ).not.toBeDefined()
      })
      ...
      it('finds last block', async () => {
        ...
        expect(
          await repository.findLatestByProject(PROJECT_B),
        ).not.toBeDefined()
      })
