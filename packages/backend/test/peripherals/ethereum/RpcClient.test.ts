...
describe(RpcClient.name, () => {
  it('divides on two calls', async () => {
    ...
    const ethereumClient = new RpcClient(provider)

    const address = EthereumAddress.random()
    const topic = 'aaaa'
    await ethereumClient.getLogsUsingBisection(address, topic, 1000, 2000)
  ...
  it('correctly divides range of two', async () => {
    ...
    const address = EthereumAddress.random()
    const topic = 'aaaa'
    await ethereumClient.getLogsUsingBisection(address, topic, 1, 2)
  ...
  it('fromBlock === toBlock', async () => {
    ...
    await expect(
      ethereumClient.getLogsUsingBisection(address, topic, 1, 1),
    ).toBeRejected()
