...
import { mockEntry, mockEntryV2 } from './addOptimismToken.test'
...
describe(aggregateReportsDailyV2.name, () => {
  const TODAY = UnixTime.now().toStartOf('day')
  const ARBITRUM = ProjectId('arbitrum')
  const OPTIMISM = ProjectId('optimism')
  const ARBITRUM_ADDRESS = EthereumAddress.random()
  ...
  it('one project one bridge one token', () => {
    ...
    const result = aggregateReportsDailyV2(reports, PROJECTS)
...
describe(saveBalancesToEntryV2.name, () => {
  it('happy path', () => {
    const projectName = 'Optimism'
    const assetId = AssetId('OP')
    const decimals = 18
    const usdTVL = 1500n
    const ethTVL = 1n
    const balance = 10n
    const entry = mockEntryV2({}, projectName)
