...
export abstract class BlockIndexerBase extends ManagedChildIndexer {
  constructor(
    protected readonly $: ActivityIndexerDeps,
    name: string,
  ) {
    super({
      ...$,
  ...
  async doUpdate(from: number, to: number): Promise<number> {
  ...
  async doInvalidate(targetHeight: number): Promise<number> {
