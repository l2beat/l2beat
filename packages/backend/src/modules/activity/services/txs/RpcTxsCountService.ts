...
import { assert, Block, ProjectId, UnixTime } from '@l2beat/shared-pure'
...
export class RpcTxsCountService {
  constructor(
    private readonly rpcClient: {
      getBlockWithTransactions: (x: number) => Promise<Block>
    },
  ...
  async getTxsCount(from: number, to: number): Promise<ActivityRecord[]> {
    const queries = range(from, to + 1).map(async (blockNumber) => {
      ...
      assert(block.timestamp, `EVM block should have timestamp: ${block.number}`)
