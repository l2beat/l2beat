...
describe(LocalStorage.name, () => {
  ...
  describe('multiple operations', () => {
    it('can store multiple items for different timestamps', async () => {
      ...
      expect(await storage.getAmount('token1', timestamp1)).toBe(1000n)
      expect(await storage.getAmount('token1', timestamp2)).toBe(2000n)
      expect(await storage.getPrice('token1', timestamp1)).toBe(100)
      expect(await storage.getPrice('token1', timestamp2)).toBe(200)
    })
    ...
    it('can store data for multiple tokens', async () => {
      ...
      expect(await storage.getAmount('token1', timestamp)).toBe(1000n)
      expect(await storage.getAmount('token2', timestamp)).toBe(2000n)
      expect(await storage.getPrice('token1', timestamp)).toBe(100)
      expect(await storage.getPrice('token2', timestamp)).toBe(200)
    })
  })
})
