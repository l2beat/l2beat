...
describeDatabase(TrackedTxsConfigsRepository.name, (knex, kysely) => {
  const oldRepo = new TrackedTxsConfigsRepository(knex, Logger.SILENT)
  const oldChildRepo = new LivenessRepository(knex, Logger.SILENT)

  const newRepo = kysely.trackedTxConfig
  const newChildRepo = kysely.liveness

  suite(oldRepo, oldChildRepo)
  suite(newRepo, newChildRepo)

  function suite(
    repository: typeof oldRepo | typeof newRepo,
    childRepository: typeof oldChildRepo | typeof newChildRepo,
  ) {
    beforeEach(async () => {
    ...
    describe(
      TrackedTxsConfigsRepository.prototype.getByProjectIdAndType.name,
      () => {
        it('should return configs for given project id and type', async () => {
          await repository.addMany(TRACKED_TXS_RECORDS)
    ...
    describe(
      TrackedTxsConfigsRepository.prototype.setUntilTimestamp.name,
      () => {
        it('updates last synced timestamp of given configuration', async () => {
