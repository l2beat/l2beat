...
interface DerivedPortalDeposit {
  sourceHash: `0x${string}`
  l2TxHash: `0x${string}`
  mint: bigint
  value: bigint
...
type ParsedTransactionDeposited = NonNullable<
  ReturnType<typeof parseTransactionDeposited>
>

// `opaqueData` is packed by the portal, so version 0 must be decoded manually.
export function derivePortalDeposit(
  params: Pick<
    ParsedTransactionDeposited,
    'from' | 'to' | 'version' | 'opaqueData'
  > & {
    blockHash: string
    logIndex: number
  },
