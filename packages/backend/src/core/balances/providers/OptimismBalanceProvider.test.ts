...
import { OptimismBalanceProvider } from './OptimismBalanceProvider'

describe(OptimismBalanceProvider.name, () => {
  describe(OptimismBalanceProvider.prototype.fetchBalances.name, () => {
    describe('if multicall can be used', () => {
      it('performs a multicall balance queries', async () => {
        ...
        const ethereumBalanceProvider = new OptimismBalanceProvider(
          optimismClient,
          multicallClient,
        )
    ...
    describe('if multicall cannot be used', () => {
      it('performs a plain calls balance queries', async () => {
        ...
        expect(optimismClient.call).toHaveBeenCalledTimes(1) // For ERC20 balanceOf call
        expect(optimismClient.getBalance).toHaveBeenCalledTimes(1) // For ETH coin
