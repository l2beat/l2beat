...
export class OptimismBalanceProvider implements BalanceProvider {
  ...
  constructor(
    ...
    private readonly multicallClient: OptimismMulticallClient,
  ) {
    assert(
  ...
  private async fetchUsingPlainCalls(
    ...
    const rpcCalls = balanceQueries.map((balanceQuery) => {
      if (this.isNativeCoin(balanceQuery.assetId)) {
        return this.optimismClient.getBalance(balanceQuery.holder, blockNumber)
      }
      ...
      return this.optimismClient.call({ to, data }, blockNumber)
    })
