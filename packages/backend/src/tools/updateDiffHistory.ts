...
async function getDiscoveryHash(projectName: string, chainName: string) {
    const chainId = ChainId.fromName(chainName)
    const configReader = new ConfigReader()
    const curDiscovery = await configReader.readDiscovery(projectName, chainId)
...
function getHashesDatabase(path: string): Record<string, string> {
    let result = {}
...
function getHashesDatabaseKey(projectName: string, chainName: string): string {
