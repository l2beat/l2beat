...
function renderRules(): string {
  return [
    ...
    "4. **Selection.** Give exactly one verdict to every item in the worklist: either its signature appears in exactly one step's `covers`, or it appears in `skips` with a reason. Leave nothing out and rule on nothing twice. Skip reasons are only: `user-activity` (state anyone can write: balances, deposits, per-user nonces, per-message or per-operation status, even if enumerable), `computation` (a pure function of its inputs), `unbounded` (state whose keys cannot be enumerated from anything available), `covered` (already produced by another step or a 0-argument getter), `not-state` (helpers, versions, interface checks).",
    ...
    "6. **Roles.** For OpenZeppelin AccessControl (`hasRole`, `getRoleAdmin`, events `RoleGranted`/`RoleRevoked`/`RoleAdminChanged`) use one `accessControl@1` step with id `accessControl` that covers both items, and fill `roleNames` with every `*_ROLE()` baseline getter: the getter's value (the bytes32 hash) as key, the getter's name as value. The zero hash is `DEFAULT_ADMIN_ROLE` automatically.",
    '7. **Reason.** `reason` is one sentence naming the writer function and its modifier (e.g. "sequencers is written only by addSequencer/removeSequencer (onlyOwner), which emit UpdateSequencer").',
    '8. **User activity is never fetched.** Balances, deposits, withdrawals, per-user nonces, message or operation status by hash, queue contents: skip them as `user-activity` even when an event would let you enumerate them.',
    '9. **Output.** Reply with exactly one JSON object that matches the plan schema in section 2, and nothing else: no prose, no code fence, no comments. Copy `contract` from the contract facts. Omit `shapeHash` unless it is given below.',
    '',
  ].join('\n')
}
