...
describe(EthereumAddress.name, () => {
  ...
  it('checks the checksum', () => {
    ...
    expect(() =>
      EthereumAddress('0xAbCdABCd12345678abcDabCd12345678ABcdaBcD'),
    ).toThrow('Invalid EthereumAddress')
  })

  it('does not accept invalid strings', () => {
    expect(() => EthereumAddress('foo')).toThrow(TypeError)
    expect(() => EthereumAddress('foo')).toThrow('Invalid EthereumAddress')
  })
  ...
  describe(EthereumAddress.isBefore.name, () => {
    it('checks ordering', () => {
      ...
      expect(EthereumAddress.isBefore(a, b)).toBe(true)
      expect(EthereumAddress.isBefore(b, a)).toBe(false)
      expect(EthereumAddress.isBefore(a, a)).toBe(false)
    })
    ...
    it('works for WETH & USDT', () => {
      ...
      expect(EthereumAddress.isBefore(weth, usdt)).toBe(true)
      expect(EthereumAddress.isBefore(usdt, weth)).toBe(false)
    })
  })
  ...
  it('properly checks ignoring case', () => {
    ...
    expect(EthereumAddress.checkIgnoringCase('test')).toBe(false)
  })
})
