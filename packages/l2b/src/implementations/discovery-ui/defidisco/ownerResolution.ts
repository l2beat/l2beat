/**
 * Shared owner resolution logic for DefidDisco (backend version)
 *
...
export function resolvePathExpression(
  ...
  try {
    ...
    if (contractRef === '$self') {
      ...
      targetContractAddress = dataAccess.getFieldValue(currentContract, fieldName)
...
export class DiscoveredDataAccess implements IContractDataAccess {
  ...
  findContract(address: string): any {
    ...
    const contract = this.discovered.entries.find((entry: any) =>
      entry.type === 'Contract' && entry.address === address
    )
