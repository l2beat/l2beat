...
let cachedConfig: DefidDiscoConfig | null = null

function loadDefidDiscoConfig(): DefidDiscoConfig {
  if (cachedConfig) {
    return cachedConfig
  }

  try {
    const configPath = join(__dirname, '../../../../../config/src/defidisco-config.json')
    const configContent = readFileSync(configPath, 'utf-8')
    cachedConfig = JSON.parse(configContent)
    return cachedConfig!
  } catch (error) {
    console.warn('Failed to load DefidDisco config, falling back to all projects:', error)
...
export function getDefiProjects(): string[] {
  const config = loadDefidDiscoConfig()
