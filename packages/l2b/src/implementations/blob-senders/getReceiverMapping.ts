...
// Mapping of blob receiver addresses (BatchInbox, SequencerInbox, etc.) to project names
// Copied from BlobsFetcher.ts receiverMapping
export const RECEIVER_MAPPING: Record<string, string> = {
...
export function getProjectByReceiver(receiver: string): string | undefined {
  return RECEIVER_MAPPING[receiver.toLowerCase()]
}

export function getReceiverName(receiver: string): string | undefined {
  const lower = receiver.toLowerCase()
  // Check L2 projects first
  const project = RECEIVER_MAPPING[lower]
  if (project) return project
  // Check non-L2 receivers
