...
function loadCache(): EventCache {
  if (fs.existsSync(CONFIG.CACHE.FILE_PATH)) {
    try {
      ...
      console.log(COLORS.trace(`Loaded cache from ${CONFIG.CACHE.FILE_PATH}. Last L2 block fetched: ${cache.lastL2BlockFetched}`));
      ...
      console.warn(COLORS.warning(`Could not read cache file ${CONFIG.CACHE.FILE_PATH}: ${error}. Starting fresh.`));
    }
  }
...
function saveCache(cache: EventCache): void {
  try {
    ...
    console.log(COLORS.trace(`Saved cache to ${CONFIG.CACHE.FILE_PATH}. Last L2 block fetched: ${cache.lastL2BlockFetched}`));
  } catch (error) {
...
function formatTimestamp(timestamp: number | ethers.BigNumber | undefined): string {
  ...
  try {
    ...
    console.warn(COLORS.warning(`Could not format timestamp ${timestamp}: ${e}`));
    return "Invalid Date";
  }
}
...
function formatValue(value: ethers.BigNumber | undefined | null): string {
  ...
  try {
    return ethers.BigNumber.from(value).isZero() ? '0' : ethers.utils.formatUnits(value, 'ether');
  } catch (e) {
...
function calculatePercentage(numerator: ethers.BigNumber, denominator: ethers.BigNumber): string {
  ...
  try {
    const numFixed = ethers.FixedNumber.fromValue(numerator, CONFIG.VOTE_TOKEN_DECIMALS);
    const denFixed = ethers.FixedNumber.fromValue(denominator, CONFIG.VOTE_TOKEN_DECIMALS);
    ...
    console.warn(COLORS.warning(`Error calculating percentage (${numerator}/${denominator}): ${e}`));
    return "Error";
  }
}
...
async function fetchL2Events<T extends ethers.BaseContract>(
  ...
  if (fromBlock > toBlock) {
    console.log(COLORS.trace(`Block range (${fromBlock}-${toBlock}) invalid or already covered for ${eventName} events.`));
    return 0;
  }

  console.log(COLORS.info(`Fetching ${eventName} events from block ${fromBlock} to ${toBlock}...`));
  ...
  try {
    ...
    console.log(COLORS.trace(`Found ${logs.length} ${eventName} events in range.`));
    ...
    for (const log of logs) {
      ...
      try {
        ...
        console.warn(COLORS.warning(`Could not parse ${eventName} log in tx ${log.transactionHash}: ${e}`));
      }
    }

    console.log(COLORS.trace(`Added ${addedCount} new unique ${eventName} events to cache.`));
  } catch (error) {
...
async function findL1MessageSentInTx(
  ...
  console.log(COLORS.info(`Fetching receipt for L2 execution tx: ${executionTxHash}...`));

  try {
    const receipt = await l2Provider.getTransactionReceipt(executionTxHash);

    if (!receipt) {
      console.error(COLORS.error(`Could not find transaction receipt for ${executionTxHash}`));
      return null;
    }
    ...
    for (const log of receipt.logs) {
      if (
        log.address.toLowerCase() === CONFIG.ADDRESSES.L1_MESSENGER.toLowerCase() && 
    ...
    console.warn(COLORS.warning(`L1MessageSent event not found in transaction ${executionTxHash}`));
    return null;
  } catch (error) {
...
async function findL1UpgradeStarted(
  ...
  console.log(COLORS.info(`Searching for L1 UpgradeStarted event with ID: ${l1ProposalHash}...`));
  ...
  try {
    ...
    const fromBlockL1 = Math.max(0, latestL1Block - CONFIG.SEARCH.L1_EVENT_BLOCK_RANGE);

    console.log(COLORS.trace(`Querying L1 blocks ${fromBlockL1} to ${latestL1Block}...`));

    const logs = await upgradeHandlerContract.queryFilter(eventFilter, fromBlockL1, latestL1Block);

    if (logs.length === 0) {
      console.warn(COLORS.warning(`UpgradeStarted event not found on L1 for ID ${l1ProposalHash} in the last ${CONFIG.SEARCH.L1_EVENT_BLOCK_RANGE} blocks.`));
      return null;
    }

    if (logs.length > 1) {
      console.warn(COLORS.warning(`Found multiple (${logs.length}) UpgradeStarted events. Using first one.`));
    }
    ...
    console.error(COLORS.error(`Error searching for L1 UpgradeStarted event: ${error}`));
    return null;
  }
}
...
function decodeL1ProposalFromL2Calldata(l2Calldata: string): UpgradeProposal | null {
  if (!l2Calldata || !l2Calldata.startsWith(CONFIG.L2_MESSENGER_SEND_SELECTOR)) {
    console.warn(COLORS.warning(`L2 Calldata does not start with L1Messenger selector (${CONFIG.L2_MESSENGER_SEND_SELECTOR})`));
    return null;
  }

  try {
    const encodedArgs = '0x' + l2Calldata.substring(CONFIG.L2_MESSENGER_SEND_SELECTOR.length);
    ...
    console.error(COLORS.error(`Error decoding L1 proposal from L2 calldata: ${e}`));
...
function comparePayloads(
  ...
  if (!l1PayloadFromL2 || !l1PayloadFromEvent) {
    console.error(COLORS.error("Cannot compare payloads, one or both are missing."));
    return false;
  }
  ...
  if (l1PayloadFromL2.executor.toLowerCase() !== l1PayloadFromEvent.executor.toLowerCase()) {
    console.error(COLORS.error(`Mismatch: Executor L2(${l1PayloadFromL2.executor}) != L1(${l1PayloadFromEvent.executor})`));
    mismatchFound = true;
  }

  // Compare salt
  if (l1PayloadFromL2.salt !== l1PayloadFromEvent.salt) {
    console.error(COLORS.error(`Mismatch: Salt L2(${l1PayloadFromL2.salt}) != L1(${l1PayloadFromEvent.salt})`));
    mismatchFound = true;
  }

  // Compare calls length
  if (l1PayloadFromL2.calls.length !== l1PayloadFromEvent.calls.length) {
    console.error(COLORS.error(`Mismatch: Call count L2(${l1PayloadFromL2.calls.length}) != L1(${l1PayloadFromEvent.calls.length})`));
    return false;
  }
  ...
  for (let i = 0; i < l1PayloadFromL2.calls.length; i++) {
    ...
    if (callL2.target.toLowerCase() !== callL1.target.toLowerCase()) {
      console.error(COLORS.error(`Mismatch (Call ${i}): Target L2(${callL2.target}) != L1(${callL1.target})`));
      mismatchFound = true;
    }

    if (!ethers.BigNumber.from(callL2.value).eq(callL1.value)) {
      console.error(COLORS.error(`Mismatch (Call ${i}): Value L2(${callL2.value.toString()}) != L1(${callL1.value.toString()})`));
      mismatchFound = true;
    }

    if (callL2.data.toLowerCase() !== callL1.data.toLowerCase()) {
      console.error(COLORS.error(`Mismatch (Call ${i}): Data L2(${callL2.data}) != L1(${callL1.data})`));
      mismatchFound = true;
    }
  }
...
async function findProposalExtendedEvent(
  ...
  if (cachedEvent) {
    console.log(COLORS.trace(`Found cached ProposalExtended event for ID ${proposalId.toString()}`));
    return cachedEvent;
  }

  console.log(COLORS.info(`Searching for ProposalExtended event for ID ${proposalId.toString()} from block ${startBlock}...`));
  ...
  try {
    ...
    if (logs.length === 0) {
      console.log(COLORS.trace(`No ProposalExtended event found for ID ${proposalId.toString()}.`));
      return null;
    }

    if (logs.length > 1) {
      console.warn(COLORS.warning(`Found multiple (${logs.length}) ProposalExtended events for ID ${proposalId.toString()}. Using the first one.`));
    }
    ...
    console.log(COLORS.success(`Found ProposalExtended event at block ${log.blockNumber}. New deadline: ${formatTimestamp(extendedData.extendedDeadline)}`));
    ...
    console.error(COLORS.error(`Error searching for ProposalExtended event: ${error}`));
    return null;
  }
}
...
function displayTimeline(
  ...
  l2States.forEach((state, index) => {
    ...
    if (state === "Active" && l2StateName === "Active" && effectiveVoteEndTs && currentL1Timestamp > 0) {
      ...
      timingInfo = COLORS.timestamp(` (${formatDuration(secondsRemaining)} left)`);
    ...
    l2Timeline += coloredState + timingInfo + 
  ...
  if (l1StateName && !l2IsOffPath) {
    ...
    if (l1StatusData && l1StatusData.creationTimestamp > 0) {
      ...
      waitOrExpiryEndTime = legalVetoEndTime + CONFIG.TIMEFRAMES.L1_WAIT_OR_EXPIRE_SECONDS;

      if (l1StatusData.securityCouncilApprovalTimestamp > 0) {
        readyTimestamp = l1StatusData.securityCouncilApprovalTimestamp + 
          CONFIG.TIMEFRAMES.L1_UPGRADE_DELAY_SECONDS;
      } else if (l1StatusData.guardiansApproval) {
        readyTimestamp = waitOrExpiryEndTime + CONFIG.TIMEFRAMES.L1_UPGRADE_DELAY_SECONDS;
      }
    }
    ...
    l1States.forEach((state, index) => {
      ...
      if (l1StatusData && currentL1Timestamp > 0) {
        if (state === "LegalVetoPeriod" && l1StateName === state && legalVetoEndTime > 0) {
          ...
          timingInfo = COLORS.timestamp(` (~${formatDuration(secondsRemaining)} left until expiry/delay)`);
        } else if (state === "ExecutionPending" && l1StateName === state && readyTimestamp > 0) {
          secondsRemaining = readyTimestamp - currentL1Timestamp;
          timingInfo = COLORS.timestamp(` (~${formatDuration(secondsRemaining)} left until ready)`);
      ...
      const isLastVisible = (index === l1States.length - 1) || 
        (l1IsOffPath && state === "Ready");

      nextArrow = !isLastVisible;
      l1Timeline += coloredState + timingInfo + 
        (nextArrow ? COLORS.divider(' -> ') : '');
    });
...
function displayVotesInfo(
  ...
  console.log(`  For:     ${COLORS.success(formatUnits(votes.forVotes, CONFIG.VOTE_TOKEN_DECIMALS))} (${forPercent}%)`);
  console.log(`  Against: ${COLORS.error(formatUnits(votes.againstVotes, CONFIG.VOTE_TOKEN_DECIMALS))} (${againstPercent}%)`);
  console.log(`  Abstain: ${COLORS.completed(formatUnits(votes.abstainVotes, CONFIG.VOTE_TOKEN_DECIMALS))} (${abstainPercent}%)`);
  console.log(`  ${COLORS.divider('--------------------')}`);
  console.log(`  Total Voted: ${COLORS.info(formatUnits(totalVotes, CONFIG.VOTE_TOKEN_DECIMALS))}`);

  if (quorumValue) {
    const quorumPercentReached = calculatePercentage(votes.forVotes, quorumValue);
    console.log(`  Quorum:      ${COLORS.detail(formatUnits(quorumValue, CONFIG.VOTE_TOKEN_DECIMALS))} (Required at Vote Start)`);
    console.log(`  Quorum Reached: ${COLORS.detail(quorumPercentReached)}% (by For votes)`);
  } else {
...
async function main(): Promise<void> {
  ...
  if (args.length !== 1) {
    console.error(COLORS.error('Usage: ts-node governance-decoder.ts <l2_proposal_id>'));
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(COLORS.error(`Invalid L2 Proposal ID format: ${l2ProposalIdInput}`));
    process.exit(1);
  }

  console.log(COLORS.heading(`--- ZKsync Era Governance Decoder ---`));
  console.log(COLORS.info(`Analyzing L2 Proposal ID: ${l2ProposalId.toString()}`));

  // --- Provider & Contract Setup ---
  if (!CONFIG.RPC.ZKSYNC2 || !CONFIG.RPC.ETHEREUM) {
    console.error(COLORS.error('Error: ZKSYNC2_RPC_URL and ETHEREUM_RPC_URL must be set in the .env file.'));
    process.exit(1);
  }
  ...
  if (!proposalCreated) {
    console.error(COLORS.error(`ProposalCreated event not found for ID ${l2ProposalId.toString()}.`));
    saveCache(cache);
    process.exit(1);
  }

  console.log(COLORS.success(`Found ProposalCreated event in L2 block ${proposalCreated.blockNumber} (Tx: ${proposalCreated.txHash})`));
  console.log(COLORS.subheading("L2 Proposal Details:"));
  console.log(`  Proposer: ${COLORS.address(proposalCreated.proposer)}`);
  console.log(`  Vote Start Time: ${COLORS.timestamp(formatTimestamp(proposalCreated.voteStart))}`);
  ...
  console.log(`  Description:\n${COLORS.trace.italic(
    proposalCreated.description.split('\n').map(l => `    ${l}`).join('\n')
  )}`);
  ...
  try {
    ...
    console.warn(COLORS.warning(`Could not fetch L2 state, votes, or quorum: ${e}`));
  }
  ...
  if (proposalExecuted) {
    console.log(COLORS.success(`Found ProposalExecuted event in L2 block ${proposalExecuted.blockNumber} (Tx: ${proposalExecuted.txHash})`));

    l1MessageSent = await findL1MessageSentInTx(l2Provider, proposalExecuted.txHash);

    if (l1MessageSent) {
      l1ProposalHash = l1MessageSent.hash;
      console.log(COLORS.subheading(`L1 Proposal Identifier (Hash): ${COLORS.hash(l1ProposalHash)}`));
    } else {
      console.error(COLORS.error(`Failed L1MessageSent lookup for tx ${proposalExecuted.txHash}.`));
    }
  } else {
    console.warn(COLORS.warning(`ProposalExecuted event not found for ID ${l2ProposalId.toString()}.`));
  }

  // --- 3. Decode L1 Payload from L2 Data ---
  let l1PayloadFromL2: UpgradeProposal | null = null;

  const messengerCallIndex = (proposalCreated.targets ?? [])
    .findIndex(t => t.toLowerCase() === CONFIG.ADDRESSES.L1_MESSENGER.toLowerCase());
  ...
  if (l1ProposalHash) {
    ...
    try {
      ...
      console.warn(COLORS.warning(`Could not query L1 state/status for ${l1ProposalHash}: ${e}`));
    }

    if (upgradeStarted) {
      console.log(COLORS.success(`Found UpgradeStarted event in L1 block ${upgradeStarted.blockNumber} (Tx: ${upgradeStarted.txHash})`));
    } else {
      console.warn(COLORS.warning(`L1 UpgradeStarted event not found for ID ${l1ProposalHash}.`));
    }
    ...
    if (l1StatusData) {
      ...
      console.log(`    Created: ${COLORS.timestamp(formatTimestamp(l1StatusData.creationTimestamp))}`);
      console.log(`    SC Approved: ${COLORS.timestamp(formatTimestamp(l1StatusData.securityCouncilApprovalTimestamp))}`);
      console.log(`    Guardians Approved: ${COLORS.value(l1StatusData.guardiansApproval.toString())}`);
      console.log(`    Guardians Extended Veto: ${COLORS.value(l1StatusData.guardiansExtendedLegalVeto.toString())}`);
      console.log(`    Executed: ${COLORS.value(l1StatusData.executed.toString())}`);
    }
  } else {
    console.log(COLORS.subheading('\n--- Step 4: L1 Proposal Status ---'));
  ...
  if (l1PayloadFromL2 && (!upgradeStarted || !payloadsMatch)) {
    ...
    if (!upgradeStarted) {
      reason = COLORS.warning("(Displaying because L1 UpgradeStarted event was not found)");
    } else if (!payloadsMatch) {
      reason = COLORS.error("(Displaying because it differs from L1 UpgradeStarted event payload)");
    }
