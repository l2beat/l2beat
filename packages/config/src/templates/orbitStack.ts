...
export function getNitroGovernance(
  ...
  return readMarkdown('templates/orbitStack/nitroGovernance.md', {
    l2CoreQuorumPercent,
    l2TimelockDelay: formatSeconds(l2TimelockDelay),
    challengeWindowSeconds: formatSeconds(challengeWindowSeconds),
    challengeGracePeriodSeconds: formatSeconds(challengeGracePeriodSeconds),
    l1TimelockDelay: formatSeconds(l1TimelockDelay),
    treasuryTimelockDelay: formatSeconds(treasuryTimelockDelay),
...
function BoLDStateValidation(
  ...
  if (existFastConfirmer) {
    categories.push({
      title: 'Fast confirmations',
      description: `The fast confirmer can confirm any pending assertion immediately, or create and confirm one in a single call, skipping the ${formatSeconds(
        assertionsChallengePeriod,
      )} challenge period. It is bound by neither the validator whitelist nor the minimum assertion period. This finalizes the state root and permits withdrawals based on it.`,
      risks: [
        {
          category: 'Funds can be stolen if',
          text: "the actor with the 'fast-confirmer' permission finalizes a malicious state root before the challenge period has passed.",
          isCritical: true,
        },
      ],
      references: [
