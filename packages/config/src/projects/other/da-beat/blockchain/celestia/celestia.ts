...
export const celestia: DaLayer = {
  ...
  technology:
    ...
    Celestia shares rows and columns are erasure coded into a 2k * 2k matrix and committed to in a Namespace Merkle Trees (NMTs).  
    Namespaced Merkle Tree is a version of a standard Merkle tree using a namespaced hash function, in which every node in the tree includes the range of namespaces of all its child nodes.
    This allows an application to request and retrieve data for a specific namespace sub-tree, while maintaining all functionalities (e.g., inclusion and range proofs) of a standard Merkle tree.

    ![Matrix proofs](/images/da-layer-technology/celestia/matrix-proofs.png#center)\n

    Ultimately, a single data root (availableDataRoot) of the Merkle tree with the row and column roots as leaves is computed, and included in the block header as the root of commitments to erasure-coded data so that individual shares in the matrix can be proven to belong to a single data root. 
    ...
    To ensure data availability, Celestia light nodes perform sampling on the 2k×2k data matrix. Each light node randomly selects a set of unique coordinates within the extended matrix and requests the corresponding data shares and Merkle proofs from full nodes.
