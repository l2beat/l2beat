...
export const celestia: DaLayer = {
  ...
  technology: {
    ...
    ## Consensus
    Celestia uses [CometBTF](https://github.com/celestiaorg/celestia-core), the canonical implementation of Tendermint consensus protocol. The consensus protocol is fork-free by construction under an honest majority of stake assumption.
    Celestia achieves finality at each block, with an average time between blocks of [12 seconds](https://celenium.io/stats?tab=Blocks).
    ## Blobs
    ...
    All data posted in a Celestia blob is divided into chunks of fixed size, called [shares](https://celestiaorg.github.io/celestia-app/shares.html), and each blob is arranged in a k * k matrix of shares. Currently [k = 64](https://github.com/celestiaorg/celestia-app/blob/b6a3ed29d977b330395cc28f9075522efaf1c49a/pkg/appconsts/initial_consts.go#L14), for a total of 4096 shares.\n

    ![Blobs matrix](/images/da-layer-technology/celestia/blobs-matrix.png#center)

    Celestia shares' rows and columns are erasure-coded into a 2k * 2k matrix and committed to in a [Namespaced Merkle Trees (NMTs)](https://github.com/celestiaorg/nmt), a version of a standard Merkle tree using a namespaced hash function. 
    ...
    Ultimately, a single data root ([availableDataRoot](https://celestiaorg.github.io/celestia-app/data_structures.html?highlight=availableDataRoot#availabledataheader)) of the Merkle tree is computed with the row and column roots as leaves. This data root is included in the block header as the root of commitments to erasure-coded data so that individual shares in the matrix can be proven to belong to a single data root.\n
    For more details on Celestia data structures, see the [Celestia documentation](https://celestiaorg.github.io/celestia-app/data_structures.html).\n
    ...
    Currently, a Celestia light node must perform a minimum of [16 samples](https://github.com/celestiaorg/celestia-node/blob/9ff58570ef86e505b718abfc755fd18643a2284c/share/availability/light/options.go#L10) before declaring that a block is available.
    ...
    For more details on DAS probabilistic analysis, see the [Fraud and Data Availability Proofs paper](https://arxiv.org/pdf/1809.09044).\n
    
    ![DAS](/images/da-layer-technology/celestia/das.png#center)

    ## Erasure Coding Proof
    Light nodes performing data availability sampling must have the guarantee that the sampled data is erasure coded correctly. In Celestia, light nodes can be notified of a maliciously encoded block through [Bad Encoding Fraud Proofs (BEFPs)](https://github.com/celestiaorg/celestia-node/blob/main/docs/adr/adr-006-fraud-service.md). Full nodes receiving invalid erasure-coded data can generate a [fraud-proof](https://github.com/celestiaorg/celestia-node/blob/9ff58570ef86e505b718abfc755fd18643a2284c/share/eds/byzantine/bad_encoding.go#L23) to be transmitted to all light and full nodes in the DA network. The proof is generated by full nodes reconstructing the original data from the block data, and verifying that the recomputed data root matches the data root of the block header. 
    Upon receiving and verifying the BEFP, all Celestia nodes should halt providing services (e.g., submitTx).

    ## L2s Data Availability
    L2s can post data to Celestia by submitting blobs through a [payForBlobs](https://github.com/celestiaorg/celestia-app/blob/b6a3ed29d977b330395cc28f9075522efaf1c49a/pkg/user/signer.go#L89) transaction. The transaction can include data as a single blob or multiple blobs, with the total maximum size determined by the maximum block size. The transaction fee is determined by the size of the data and the current gas price. 
    Applications can then [retrieve](https://github.com/celestiaorg/celestia-node/blob/9ff58570ef86e505b718abfc755fd18643a2284c/share/eds/retriever.go#L60) the data by querying the Celestia blockchain for the data root of the blob and the namespace of the application. The data can be [reconstructed](https://github.com/celestiaorg/celestia-node/blob/9ff58570ef86e505b718abfc755fd18643a2284c/share/eds/retriever.go#L177) by querying the Celestia network for the shares of the data matrix and reconstructing the data using the erasure coding scheme.
