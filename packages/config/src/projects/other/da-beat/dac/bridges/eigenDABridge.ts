...
export const eigenDAbridge = {
  ...
  technology: {
    ...
    The [checkSignatures()](https://etherscan.io/address/0x58fDE694Db83e589ABb21A6Fe66cb20Ce5554a07#code#F5#L96) function checks that the signature of all signers plus non-signers is equal to the registered quorum aggregated public key from the BLS registry. The quorum aggregated public key gets updated every time an operator is registered.
    ...
    Although thresholds are not enforced onchain by the confirmBatch method, the [minimum thresholds](https://etherscan.io/address/0x58fDE694Db83e589ABb21A6Fe66cb20Ce5554a07#code#F8#L44) that the disperser would need to reach before relaying the batch commitment to Ethereum are set to ${quorum1Threshold}% of the registered stake for the ETH quorum and ${quorum2Threshold}% for the EIGEN token quorum. Meeting these dispersal thresholds allows the system to tolerate up to ${quorum1AdversaryThreshold}% (quorum 1) and ${quorum2AdversaryThreshold}% (quorum 2) of the total stake being [adversarial](https://etherscan.io/address/0x58fDE694Db83e589ABb21A6Fe66cb20Ce5554a07#code#F8#L39), achieving this with approximately 4.5 data redundancy.  
    The quorum thresholds are set on the EigenDAServiceManager contract and can be changed by the contract owner.
    There is a maximum of ${operatorSetParamsQuorum1[0]} operators that can register for the ETH quorum and ${operatorSetParamsQuorum2[0]} for the EIGEN token quorum. Once the cap is reached, new operators must have 10% more weight than the lowest-weighted operator to join the active set. Entering the quorum is [subject to the approval](https://etherscan.io/address/0xdcabf0be991d4609096cce316df08d091356e03f#code#F1#L162) of the churn approver. Operators can be [ejected](https://etherscan.io/address/0xdcabf0be991d4609096cce316df08d091356e03f#code#F1#L364) from a quorum by the ejectors without delay should they violate the Service Legal Agreement (SLA). \n
