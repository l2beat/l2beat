Generated with discovered.json: 0x67ce4a7ff1bc632b50576ab8e080b1748b9aaf00
...
- KMSVerifier v0.2.0 -> v0.3.0 ([implementation diff](https://disco.l2beat.com/diff/eth:0xd0d0C7E1bc1E2F6Cd00E3b4B1083DdD9969155FD/eth:0x390683dEa46a61786f4F0807bf3559038Cf43Fa4)): all KMS context management (signer sets, thresholds, context create/destroy) is removed from the verifier and moved to the new ProtocolConfig contract. The KMSVerifier is now a stateless proof checker whose signer- and threshold-reads delegate to ProtocolConfig; its own context storage is abandoned as deprecated. Public decryption proofs can still select any non-destroyed context via extraData v1.
- New ProtocolConfig v0.1.0 (proxy eth:0xD8236B57394f90726b26aB25D38CeAC776E1a7C4, impl eth:0x25c76298Fb4d15bFADce4798519a7B37ac0076E7): ACL-owner-governed registry of KMS nodes (signer + tx-sender + metadata) and contexts, now with four separate per-context thresholds: publicDecryption (7), userDecryption (9), kmsGen and mpc. It was initialized via `initializeFromMigration` preserving the existing context ID and the same 13 KMS signers, so exactly one context is live. Risk-relevant change: where KMSVerifier v0.2 contexts had immutable thresholds (retuning required a full context rotation), the ACL owner can now retune the thresholds of any live context — including the current one — in place, mirroring GatewayConfig v0.6.0 on the Gateway.
...
- ACL v0.3.0 -> v0.4.0 ([implementation diff](https://disco.l2beat.com/diff/eth:0x3F6D970d30E1FFE9657aa8072C82dA10eef1c3D6/eth:0x9A017e0ba4c19A325D5a89f5dd2112e605E85418)): adds wildcard user-decryption delegation — a user can delegate user decryption of their handles across all app contracts at once via the sentinel contractAddress 0xFFfF...FFfF (high-trust grant, but it does not bypass per-handle `allow`). Delegation expiry check tightened from >= to >. The PauserSet interface gains `swapPauser` (ACL-owner-only).
...
All four contracts were re-templated (ZamaKMSVerifier_v0_3_0, ZamaACL_v0_4_0, ZamaFHEVMExecutor_v0_4_0, ZamaHCULimit_v0_3_0), and ProtocolConfig and the Ethereum KMSGeneration were added to discovery with new templates (ZamaProtocolConfig, ZamaKMSGeneration), including the 13 KMS node transaction senders that submit key- and CRS-generation responses.
