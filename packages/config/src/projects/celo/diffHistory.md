Generated with discovered.json: 0xdcb5716ddf2bb7a32c9880c99c3b798dd7e7b2bc

# Diff at Tue, 28 Apr 2026 15:43:23 GMT:

- author: vincfurc (<vincfurc@users.noreply.github.com>)
- comparing to: main@0695512a70f7175257fb7756eb2008702d3f0dc5 block: 1777038237
- current timestamp: 1777390883

## Description

OPSuccinctFaultDisputeGame implementation rotation for `gameType=42`. DisputeGameFactory now points to `0xA35d2A7F...` (replaces the previously-tracked `0xE7bd695d...`, which was deleted from discovery).

The new game keeps `version = 2.0.0` and the same `rollupConfigHash` (`0x0961504b...`). The change is a **vkey rotation** of the SP1 verification keys:
- `aggregationVkey`: `0x004f4bbc...` → `0x003ff44d...`
- `rangeVkeyCommitment`: `0x1fffeb5a...` → `0x5b3aba7b...`

Both new vkeys added to `programHashes.ts` as `OP_SUCCINCT_FDP_AGG_EIGENDA` / `OP_SUCCINCT_FDP_RANGE_EIGENDA` entries with `notVerified` status (matching the prior celo entries — exact source tag in `celo-org/op-succinct` not yet identified).

OPSuccinctFaultDisputeGame: [diff](https://disco.l2beat.com/diff/eth:0xE7bd695d6A17970A2D9dB55cfeF7F2024d630aE1/eth:0xA35d2A7F365b42EcFCB7Db9240c3973Fc8e65139)
...
+        "eth:0xA35d2A7F365b42EcFCB7Db9240c3973Fc8e65139"
    }
```

```diff
+   Status: CREATED
    contract OPSuccinctFaultDisputeGame (eth:0xA35d2A7F365b42EcFCB7Db9240c3973Fc8e65139)
    +++ description: Logic of the dispute game. When a state root is proposed, a dispute game contract is deployed. Challengers can use such contracts to challenge the proposed state root.
```
