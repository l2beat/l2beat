...
export const privacyBoostAdversaries = definePrivacyAdversaries({
  ...
  cells: {
    publicObserver: {
      sentiment: 'good',
      exposure:
        'Transfers inside publish only encrypted notes, and the link between deposit and withdrawal is hidden. Deposits and withdrawals show address, token and amount.',
      advice:
    ...
    chainAnalyst: {
      sentiment: 'bad',
      exposure:
        'Once the operator heartbeat, one address cycling a fixed amount on a fixed cadence, is filtered out, the real anonymity set is tiny. Every exit is the note it spends minus the fixed fee, so amount matching alone pairs a deposit with its exit.',
      interior: {
        sender: 'atRisk',
        recipient: 'atRisk',
        amount: {
