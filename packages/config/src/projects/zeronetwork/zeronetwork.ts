...
export const zeronetwork: ScalingProject = zkStackL2({
  ...
  // daTracking comes from the zkStackDaTracking handler output in
  // discovered.json (see config.jsonc), which reconstructs the full era
  // history from on-chain events. The previously hand-written arrays are
  // preserved: both their identities survive in the handler output, plus the
  // precise pre-rotation era (blocks 22120195-24879563) is resurrected.
