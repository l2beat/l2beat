...
contract SystemConfig is OwnableUpgradeable, ISemver, IGasToken {
    ...
    struct SequencerPubkeyInfo {
        uint256 l2Height;
        bytes32 sequencerPubkey;
    }
    SequencerPubkeyInfo[] public sequencerPubkeyInfos;
    ...
    /// @notice Emitted when set new sequencer pubkey.
    /// @param l2Height L2 block height.
    /// @param sequencerPubkey New sequencer pubkey.
    event SequencerPubkeyInfoAdded(uint256 indexed l2Height, bytes32 sequencerPubkey);

    /// @notice Emitted when remove the last sequencer pubkey.
    event SequencerPubkeyInfoRemoved(uint256 indexed l2Height, bytes32 sequencerPubkey);
    ...
    /// @notice Internal setter for the gas paying token address.
    ///         Currently Soon adopted const gas paying token, which is ETH.
    ...
    /// @notice Set the L2 sequencer pubkey on specified l2 height
    /// @param l2Height L2 block height.
    /// @param sequencerPubkey New sequencer pubkey.
    function setSequencerPubkey(uint256 l2Height, bytes32 sequencerPubkey) external onlyOwner {
        uint256 currentLength = sequencerPubkeyInfos.length;
        if (currentLength == 0) {
            require(l2Height == 0, "SystemConfig: first sequencer info block height should be 0");
        } else {
            SequencerPubkeyInfo memory latestInfo = sequencerPubkeyInfos[currentLength - 1];
            require(l2Height > latestInfo.l2Height, "SystemConfig: invalid l2 height");
            require(sequencerPubkey != latestInfo.sequencerPubkey, "SystemConfig: same pubkey");
        }

        sequencerPubkeyInfos.push(SequencerPubkeyInfo({
            l2Height: l2Height,
            sequencerPubkey: sequencerPubkey
        }));

        emit SequencerPubkeyInfoAdded(l2Height, sequencerPubkey);
    }

    /// @notice Remove the last item of sequencer pubkey infos
    function removeLastSequencerPubkey() external onlyOwner {
        uint256 currentLength = sequencerPubkeyInfos.length;
        require(currentLength > 0, "SystemConfig: empty sequencer pubkey infos");
        SequencerPubkeyInfo memory latestInfo = sequencerPubkeyInfos[currentLength - 1];
        sequencerPubkeyInfos.pop();

        emit SequencerPubkeyInfoRemoved(latestInfo.l2Height, latestInfo.sequencerPubkey);
    }

    /// @notice Get all sequencer pubkey info
    function getSequencerPubkeyInfos() external view returns(SequencerPubkeyInfo[] memory) {
        return sequencerPubkeyInfos;
    }
}