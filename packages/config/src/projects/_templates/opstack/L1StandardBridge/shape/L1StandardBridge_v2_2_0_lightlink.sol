...
abstract contract StandardBridge is Initializable {
    ...
    modifier onlyEOA() {
        require(
            // !Address.isContract(msg.sender), This isnt available in current version of OpenZeppelin
            // But it just wraps the below code:
    ...
    function finalizeBridgeETH(
        ...
        require(
            ...
            "StandardBridge: cannot bridge ETH with custom gas token"
        );
        require(
            msg.value == _amount,
    ...
    function finalizeBridgeERC20(
        ...
        require(paused() == false, "StandardBridge: paused");
        if (_isLightLinkMintableERC20(_localToken)) {
            require(
                _isCorrectTokenPair(_localToken, _remoteToken),
                "StandardBridge: wrong remote token for LightLink Mintable ERC20 local token"
            );

            LightLinkMintableERC20(_localToken).mint(_to, _amount);
        } else {
    ...
    function _initiateBridgeERC20(
        ...
        if (_isLightLinkMintableERC20(_localToken)) {
            ...
            LightLinkMintableERC20(_localToken).burn(_from, _amount);
        } else {
    ...
    /// @notice Checks if a given address is an LightLinkMintableERC20. Not perfect, but good enough.
    ///         Just the way we like it.
    /// @param _token Address of the token to check.
    /// @return True if the token is an LightLinkMintableERC20.
    function _isLightLinkMintableERC20(
        address _token
    ) internal view returns (bool) {
        return
            ERC165Checker.supportsInterface(
                _token,
                type(ILegacyMintableERC20).interfaceId
            ) ||
            ERC165Checker.supportsInterface(
                _token,
                type(ILightLinkMintableERC20).interfaceId
            );
    }

    /// @notice Checks if the "other token" is the correct pair token for the LightLinkMintableERC20.
    ///         Calls can be saved in the future by combining this logic with
    ///         `_isLightLinkMintableERC20`.
    /// @param _mintableToken LightLinkMintableERC20 to check against.
    /// @param _otherToken    Pair token to check.
    /// @return True if the other token is the correct pair token for the LightLinkMintableERC20.
    function _isCorrectTokenPair(
        ...
        if (
            ...
            return
                _otherToken == ILegacyMintableERC20(_mintableToken).l1Token();
        } else {
            return
                _otherToken ==
                ILightLinkMintableERC20(_mintableToken).remoteToken();
        }
    }
...
contract L1StandardBridge is StandardBridge {
        ...
        returns (address addr_, uint8 decimals_)
    {
        ...
        // Temporarily hardcode the gas paying token to ETH
