Generated with discovered.json: 0xe6f540786793ba0d02269a14b198cc0418c2275f

# Diff at Wed, 23 Sep 2026 05:46:29 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@2e9174e8edc4a3b646f958a1d6e0d4360abec40d block: 1758702279
...
```diff
    contract AddressManager (eth:0x8c5D64d10394cFa070066e70Ec19E67398b4dABE) [opstack/AddressManager] {
...
- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@e2faf827d006bceee5fb0904599ba325066c7674 block: 1758702279
- current timestamp: 1758702279
...
Generated with discovered.json: 0xe56fd9defe44d7d8e36f2b8306de4bc03032300f

# Diff at Tue, 09 Jun 2026 12:43:33 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@ae67a38d37457ad735e5d55080d2e5479d5df7dc block: 1758702279
- current timestamp: 1758702279
...
```diff
    EOA  (eth:0x6017f75108f251a488B045A7ce2a7C15b179d1f2) {
    +++ description: None
...
-        {"permission":"challenge","from":"eth:0x66CC916Ed5C6C2FA97014f7D1cD141528Ae171e4","role":".challenger"}
      receivedPermissions.1:
-        {"permission":"challenge","from":"eth:0x66CC916Ed5C6C2FA97014f7D1cD141528Ae171e4","role":".CHALLENGER"}
      receivedPermissions.2:
-        {"permission":"guard","from":"eth:0x36cb65c1967A0Fb0EEE11569C51C2f2aA1Ca6f6D","role":".guardian"}
      receivedPermissions.3:
-        {"permission":"guard","from":"eth:0x61ca43CB037aC9181d8Fa5CD0073dC314065Ccc4","role":".guardian"}
      receivedPermissions.1:
+        {"permission":"interact","from":"eth:0x36cb65c1967A0Fb0EEE11569C51C2f2aA1Ca6f6D","description":"Allowed to pause withdrawals. In op stack systems with a proof system, the Guardian can also blacklist dispute games and set the respected game type (permissioned / permissionless).","role":".guardian"}
      receivedPermissions.2:
+        {"permission":"interact","from":"eth:0x61ca43CB037aC9181d8Fa5CD0073dC314065Ccc4","description":"Allowed to pause withdrawals. In op stack systems with a proof system, the Guardian can also blacklist dispute games and set the respected game type (permissioned / permissionless).","role":".guardian"}
      receivedPermissions.3:
+        {"permission":"interact","from":"eth:0x66CC916Ed5C6C2FA97014f7D1cD141528Ae171e4","description":"Allowed to challenge or delete state roots proposed by a Proposer.","role":".challenger"}
...
```diff
    EOA  (eth:0xFb90465f3064fF63FC460F01A6307eC73d64bc50) {
    +++ description: None
      receivedPermissions.0.description:
...
Generated with discovered.json: 0x188629470ea795da5d15442132513166bd13d448
...
- author: Mateusz Radomski (<radomski.main@protonmail.com>)
- comparing to: main@b6437082b3ea8fb0d97f4474b1c3452a1ce271b0 block: 1758702279
- current timestamp: 1758702279
...
```diff
    contract OptimismMintableERC20Factory (eth:0x11FE3be54aC01C13Dd985cE2BdD10eD77e1376cc) {
    +++ description: A helper contract that generates OptimismMintableERC20 contracts on the network it's deployed to. OptimismMintableERC20 is a standard extension of the base ERC20 token contract designed to allow the L1StandardBridge contracts to mint and burn tokens. This makes it possible to use an OptimismMintableERC20 as this chain's representation of a token on the host chain, or vice-versa.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
```

```diff
    contract L1CrossDomainMessenger (eth:0x126bcc31Bc076B3d515f60FBC81FddE0B0d542Ed) {
    +++ description: Sends messages from host chain to this chain, and relays messages back onto host chain. In the event that a message sent from host chain to this chain is rejected for exceeding this chain's epoch gas limit, it can be resubmitted via this contract's replay function.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract ProxyAdmin (eth:0x13Fe62cB24aEa5afd179F20D362c056c3881ABcA) {
    +++ description: None
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    ...
    +++ description: None
      deployerAddress:
+        "eth:0xa448833bEcE66fD8803ac0c390936C79b5FD6eDf"
    }
```

```diff
    contract SystemConfig (eth:0x34a9f273cbD847d49c3De015FC26c3E66825f8b2) {
    +++ description: Contains configuration parameters such as the Sequencer address, gas limit on this chain and the unsafe block signer address.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract L1StandardBridge (eth:0x34C0bD5877A5Ee7099D0f5688D65F4bB9158BDE2) {
    +++ description: The main entry point to deposit ERC20 tokens from host chain to this chain.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract EtherRouter (eth:0x5acAf61d339dd123e60ba450Ea38fbC49445007C) {
    +++ description: ETH deposited by users via the FraxEtherMinter is forwarded to this contract and then routed further to either lending pools or AMOs.
      deployerAddress:
+        "eth:0x625e700125FF054f75e5348497cBFab1ee4b7A40"
    }
```

```diff
    contract Frax Ether Token (eth:0x5E8422345238F34275888049021821E8E08CAa1f) {
    +++ description: frxETH token contract. Fraxtal uses Frax Ether as the designated gas token, allowing users to pay for blockspace with frxETH.
      deployerAddress:
+        "eth:0x4600D3b12c39AF925C2C07C487d31D17c1e32A35"
    }
```

```diff
    contract SuperchainConfig (eth:0x61ca43CB037aC9181d8Fa5CD0073dC314065Ccc4) {
    +++ description: This is NOT the shared SuperchainConfig contract of the OP stack Superchain but rather a local fork. It manages the `PAUSED_SLOT`, a boolean value indicating whether the local chain is paused, and `GUARDIAN_SLOT`, the address of the guardian which can pause and unpause the system.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract L2OutputOracle (eth:0x66CC916Ed5C6C2FA97014f7D1cD141528Ae171e4) {
    +++ description: Contains a list of proposed state roots which Proposers assert to be a result of block execution. Currently only the PROPOSER address can submit new state roots.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract frxETH Minter (eth:0x7Bc6bad540453360F744666D625fec0ee1320cA3) {
    +++ description: Accepts user-supplied ETH to convert it to frxETH using auxiliary contracts like the EtherRouter.
      deployerAddress:
+        "eth:0x625e700125FF054f75e5348497cBFab1ee4b7A40"
    }
...
```diff
    contract Frax Multisig  (eth:0x8306300ffd616049FD7e4b0354a64Da835c1A81C) {
    +++ description: None
      deployerAddress:
+        "eth:0xf4E1d185666a624099298FcC42C50ba662DC7e52"
    }
```

```diff
    contract FrxEthTimelock (eth:0x8412ebf45bAC1B340BbE8F318b928C466c4E39CA) {
...
```diff
    contract AddressManager (eth:0x8c5D64d10394cFa070066e70Ec19E67398b4dABE) {
    +++ description: Legacy contract used to manage a mapping of string names to addresses. Modern OP stack uses a different standard proxy system instead, but this contract is still necessary for backwards compatibility with several older contracts.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    contract L1ERC721Bridge (eth:0xa9B5Fb84B7aeAF0D51C95DB04a76B1D4738D0eC5) {
    +++ description: Used to bridge ERC-721 tokens from host chain to this chain.
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
```diff
    ...
    +++ description: None
      deployerAddress:
+        "eth:0x869068bd7287573333a8C1ea7e07Fb8F2aD5d5C0"
    }
```

```diff
    contract frxETH Minter (eth:0xbAFA44EFE7901E04E39Dad13167D089C559c1138) {
    +++ description: Accepts user-supplied ETH and converts it to frxETH.
      deployerAddress:
+        "eth:0x4600D3b12c39AF925C2C07C487d31D17c1e32A35"
    }
...
```diff
    contract Fraxtal Multisig 1 (eth:0xe0d7755252873c4eF5788f7f45764E0e17610508) {
    +++ description: None
      deployerAddress:
+        "eth:0xE7c147CD1A7c05a6e73217645547582024E87a9B"
    }
...
Generated with discovered.json: 0xa9304bb6ad6bb1e245fdfa4f43775797c50069ed

# Diff at Tue, 04 Nov 2025 11:32:43 GMT:

- author: Mateusz Radomski (<radomski.main@protonmail.com>)
- comparing to: main@9ff7b62a511791b99f61b604fb6b56e4ea223bb0 block: 1758702279
- current timestamp: 1758702279
