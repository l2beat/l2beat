...
### Governance
There is no governance: no multisig, timelock or DAO. Every role is an externally owned account, and {{adminKeyPhrase}}. That key can, with no delay, pause the token and the redemption queue, blacklist any holder, add minters that create unbacked {{symbol}}, replace the oracle, operator and ethReceiver, and rewrite the rate limiter. Separately, {{upgradePhrase}}; upgrades are immediate.
