...
export function opStackSmartCommon(
...
export function opStackSmartL2(templateVars: OpStackConfigL2): Layer2 {
  const sequencerInbox = EthereumAddress(
    templateVars.discovery.getContractValue('SystemConfig', 'sequencerInbox'),
  )
  ...
  return {
    type: 'layer2',
    ...opStackSmartCommon(templateVars),
    display: {
      architectureImage,
...
export function opStackSmartL3(templateVars: OpStackConfigL3): Layer3 {
  const optimismPortalTokens = [
    'ETH',
    ...(templateVars.nonTemplateOptimismPortalEscrowTokens ?? []),
  ]
