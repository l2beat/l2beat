        ...
        (() => {
          ...
          if (!isPostBoLD) {
            ...
            return {
              ...
              description:
                commonDescription +
                ` The time bound is defined to be the minimum between ${formatSeconds(selfSequencingDelaySeconds)} and the time left in the delay buffer. The delay buffer gets replenished over time and gets consumed every time the sequencer doesn't timely process a message. Only messages processed with a delay of ${formatSeconds(buffer.threshold * blockNumberOpcodeTimeSeconds)} or more consume the buffer. The buffer is capped at ${formatSeconds(buffer.max * blockNumberOpcodeTimeSeconds)}. The replenish rate is currently set at ${formatSeconds((buffer.replenishRateInBasis / basis) * 1200)} every ${formatSeconds(1200)}. Even if the buffer is fully consumed, the minimum delay is set to ${formatSeconds(buffer.threshold * blockNumberOpcodeTimeSeconds)}.`,
              references: [],
              risks: [],
            } as ProjectTechnologyChoice
          }
