...
The protocol fee is switched on: since the UNIfication proposal the factory owner role sits with the V3OpenFeeAdapter, where governance sets the fee schedule (currently 1/{{defaultProtocolFeeDenominator}} of LP fees per side by default, 1/{{tier03ProtocolFeeDenominator}} on the 0.3% tier). Anyone can apply the scheduled fee to pools and sweep accrued protocol fees. Swept fees land in the TokenJar and can only be released by burning UNI through the Firepit, so protocol revenue can be permissionlessly exchanged for UNI taken out of circulation.
