...
Liquidity providers commonly mint through the NonfungiblePositionManager, which wraps each position as a transferable ERC-721 NFT and does the fee accounting per token ID. For positions minted this way, the NFT holder uses the manager to add or remove liquidity and collect fees. The manager is a convenience, not a gatekeeper: a pool's mint function is open to any contract that pays for the deposit in the pool's callback, and the owner of a direct pool position can withdraw liquidity and collect fees by calling the pool's burn and collect functions directly.

### Swapping

Swaps normally go through a router: SwapRouter, SwapRouter02 (which also reaches Uniswap v2 pools), or the UniversalRouter, which can pull tokens through Permit2 instead of direct ERC-20 allowances to the router. Which pools a trade crosses is computed off-chain by the interface or a routing API, and the chain never verifies that the chosen route was the best one. Router swap calls encode a minimum output or maximum input, but deadline protection depends on the entrypoint: the original SwapRouter includes a deadline, while SwapRouter02 and UniversalRouter provide deadline-bearing wrappers or overloads alongside paths with no deadline. A transaction sent through a no-deadline path can execute later as long as its amount bound still passes. A bad route can therefore execute anywhere down to the signed minimum, so the slippage tolerance, not the quote, is the real protection; anything below it reverts. Swapping without any router is equally possible for a contract that pays the pool in its swap callback.
...
Over pool behavior, governance holds exactly two control knobs, both exercised through the factory owner role. It can enable new fee tiers, each permanently bound to its tick spacing and never removable. And it can switch on the protocol fee: pool code requires each side's share to be zero or between 1/10 and 1/4 of that side's LP fees, so at most a quarter of swap fees, and never principal, can be diverted.

The protocol fee is switched on: since the UNIfication proposal the factory owner role sits with the V3OpenFeeAdapter, where governance sets the fee schedule (currently {{defaultProtocolFee}} by default and {{tier03ProtocolFee}} on the 0.3% tier). Anyone can apply the scheduled fee to pools and, while the adapter remains factory owner, sweep accrued protocol fees to its hardcoded TokenJar.

Under the current configuration, the Firepit is the TokenJar's releaser. Anyone can burn the configured threshold of UNI by sending it permanently to 0xdead, then release up to 20 selected TokenJar balances to a chosen recipient. This disposal path is governed rather than immutable: after the timelock, governance can replace the TokenJar releaser, change the Firepit threshold (including to zero), or transfer factory ownership so a new owner can collect accrued protocol fees to another recipient. These controls apply only to protocol-fee accounting and cannot withdraw liquidity providers' principal.
