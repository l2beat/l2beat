...
export const ccip: BaseProject = {
  ...
  interopConfig: {
    ...
    detailedDescription: `
    # v1.6 Architecture
    All crosschain messages in Chainlink CCIP are validated and signed by a fixed signer set (OCR), currently equivalent to a ${ocrCommitF}/${ocrCommitN} multisig. The router is the main entrypoint of the system, used to send messages to other chains or receive them.
\n\n
    Outgoing messages go through an "OnRamp" contract, which is tasked to perform fee estimation through a "FeeQuoter" contract, and fetch the proper token pool to ultimately redirect funds through a "TokenAdminRegistry" contract. The pool either locks or burns the funds, depending on the specific pool contract logic. The OnRamp may also enable "filterers" to exclude the relaying of messages based on sender or content.
\n\n
    ...
    # OCR set updates
    The OCR set corresponds to a ${ocrCommitF}/${ocrCommitN} multisig. The owner of the OffRamp can arbitrarily update the threshold and signer set used to validate messages, so trust in this permissioned actor is required. The actual permission structure behind this role is complex, see the Permissions section for more details.
