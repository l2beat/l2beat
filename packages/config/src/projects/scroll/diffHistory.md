Generated with discovered.json: 0xd83d03b69a0ed58d01c3d9f30f7f42d12121f160
...
discovery. Values are for block 1789723805 (main branch discovery), not current.
...
```diff
    ...
    +++ description: Contains the array of queued L1 -> L2 messages, either appended using the L1ScrollMessenger or the EnforcedTxGateway.
      fieldMeta:
-        {"$admin":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"scrollChain":{"severity":"HIGH"},"enforcedTxGateway":{"severity":"HIGH"},"systemConfig":{"severity":"HIGH"},"messageQueueV1":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: None
      fieldMeta:
-        {"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: Contract used to send L1 -> L2 and relay messages from L2. It allows to replay failed messages and to drop skipped messages. L1 -> L2 messages sent using this contract pay for L2 gas on L1 and will have the aliased address of this contract as the sender.
      fieldMeta.enforcedTxGateway:
-        {"severity":"HIGH"}
      fieldMeta.messageQueueV2:
-        {"severity":"HIGH"}
      fieldMeta.messageQueueV1:
-        {"severity":"HIGH"}
      fieldMeta.rollup:
-        {"severity":"HIGH"}
      fieldMeta.owner:
...
```diff
    ...
    +++ description: Contract used to bridge WETH from L1 to L2.
      fieldMeta:
-        {"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"},"l2WETH":{"severity":"HIGH"}}
    }
```
...
+++ description: L1-to-L2 token mapping used to validate custom-token deposits and withdrawals.
+++ severity: HIGH
      values.tokenMapping:
+        {"eth:0xD9A442856C234a39a81a089C06451EBAa4306a72":"eth:0xc4d46E8402F476F269c379677C99F18E22Ea030e"}
      fieldMeta.router:
-        {"severity":"HIGH"}
      fieldMeta.messenger:
-        {"severity":"HIGH"}
...
-        {"severity":"HIGH"}
      fieldMeta.counterpart:
-        {"severity":"HIGH"}
      fieldMeta.tokenMapping:
+        {"severity":"HIGH","description":"L1-to-L2 token mapping used to validate custom-token deposits and withdrawals."}
    }
```
...
```diff
    ...
    +++ description: Contract used to bridge ERC20 tokens from L1 to L2. It uses a fixed token list.
      fieldMeta:
-        {"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"l2TokenFactory":{"severity":"HIGH"},"l2TokenImplementation":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"}}
    }
```
...
+++ description: Current USDC minters and their maximum mint allowances.
+++ severity: HIGH
      values.minterAllowances:
+        {"scr:0x33B60d5Dd260d453cAC3782b0bDC01ce84672142":{"minter":"scr:0x33B60d5Dd260d453cAC3782b0bDC01ce84672142","minterAllowedAmount":"115792089237316195423570985008687907853269984665640564039457584007913129639935"}}
+++ description: USDC minter removals.
+++ severity: HIGH
      values.removedMinters:
+        []
      fieldMeta.owner:
-        {"severity":"HIGH"}
...
-        {"severity":"HIGH"}
      fieldMeta.$admin:
-        {"severity":"HIGH"}
      fieldMeta.minterAllowances:
+        {"severity":"HIGH","description":"Current USDC minters and their maximum mint allowances."}
      fieldMeta.removedMinters:
+        {"severity":"HIGH","description":"USDC minter removals."}
    }
```
...
```diff
    ...
    +++ description: Used to propose and manage onchain governance proposals.
      fieldMeta.PROPOSAL_TYPES_CONFIGURATOR:
-        {"severity":"HIGH"}
      fieldMeta.token:
-        {"severity":"HIGH"}
      fieldMeta.admin:
...
```diff
    ...
    +++ description: None
      values.counterpart:
-        "scr:0xf1AF3b23DE0A5Ca3CAb7261cb0061C0D779A5c7B"
...
```diff
    ...
    +++ description: Used to append messages to the L2MessageQueue from the L2ScrollMessenger.
      fieldMeta:
-        {"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"}}
    }
```
...
+++ description: L2-to-L1 token mapping used to validate custom-token deposits and withdrawals.
+++ severity: HIGH
      values.tokenMapping:
+        {}
      fieldMeta.router:
...
-        {"severity":"HIGH"}
      fieldMeta.tokenMapping:
+        {"severity":"HIGH","description":"L2-to-L1 token mapping used to validate custom-token deposits and withdrawals."}
    }
```
...
```diff
    ...
    +++ description: None
      fieldMeta:
-        {"WETH":{"severity":"HIGH"},"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"},"l1WETH":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: Contract of the ERC20 standard token used by the ERC20 factory.
      fieldMeta:
-        {"counterpart":{"severity":"HIGH"},"gateway":{"severity":"HIGH"}}
    }
```
...
+        {"permission":"interact","from":"scr:0x2f3F2054776bd3C2fc30d750734A8F539Bb214f0","description":"can configure contract settings such as voting delay, quorum, contract manager.","role":".admin"}
      critical:
...
```diff
    ...
    +++ description: Contract used to withdraw ERC20 tokens on L2 and finalize deposit the tokens from L1.
      fieldMeta:
-        {"tokenFactory":{"severity":"HIGH"},"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"}}
    }
```
...
Generated with discovered.json: 0x1b1f4244d103fef13ea2f6063a0e51a02d1325d2

# Diff at Mon, 21 Sep 2026 11:24:06 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@231e4a5828ee5ff5a863f7b80215466ca39a5b1e block: 1789723805
- current timestamp: 1789723805
...
```diff
    ...
    +++ description: Contract used to send L1 -> L2 and relay messages from L2. It allows to replay failed messages and to drop skipped messages. L1 -> L2 messages sent using this contract pay for L2 gas on L1 and will have the aliased address of this contract as the sender.
      fieldMeta:
+        {"paused":{"severity":"MEDIUM"},"enforcedTxGateway":{"severity":"HIGH"},"messageQueueV2":{"severity":"HIGH"},"messageQueueV1":{"severity":"HIGH"},"rollup":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: The main contract of the Scroll chain. Allows to post transaction data and state roots, along with proofs. Sequencing and proposing are behind a whitelist unless enforcedBatchMode is activated.
      fieldMeta:
+        {"paused":{"severity":"MEDIUM"},"isEnforcedModeEnabled":{"severity":"HIGH"},"systemConfig":{"severity":"HIGH"},"messageQueueV2":{"severity":"HIGH"},"messageQueueV1":{"severity":"HIGH"},"verifier":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: Contract used to bridge USDC tokens from L1 to L2.
      fieldMeta:
+        {"withdrawPaused":{"severity":"MEDIUM"},"depositPaused":{"severity":"MEDIUM"},"circleCaller":{"severity":"HIGH"},"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"},"l2USDC":{"severity":"HIGH"}}
    }
```
...
```diff
    contract L1GatewayRouter (eth:0xF8B1378579659D8F7EE5f3C929c2f3E332E41Fd6) [scroll/L1GatewayRouter] {
    +++ description: Main entry point for depositing ETH and ERC20 tokens, which are then forwarded to the correct gateway.
      fieldMeta.ethGateway:
+        {"severity":"HIGH"}
      fieldMeta.defaultERC20Gateway:
+        {"severity":"HIGH"}
      fieldMeta.owner:
+        {"severity":"HIGH"}
      fieldMeta.$admin:
...
```diff
    ...
    +++ description: Contract of the USDC token on Scroll.
      fieldMeta:
+        {"paused":{"severity":"MEDIUM"},"rescuer":{"severity":"MEDIUM"},"pauser":{"severity":"MEDIUM"},"blacklister":{"severity":"MEDIUM"},"owner":{"severity":"HIGH"},"masterMinter":{"severity":"HIGH"},"implementation":{"severity":"HIGH"},"admin":{"severity":"HIGH"},"$admin":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: Used to propose and manage onchain governance proposals.
      fieldMeta:
+        {"PROPOSAL_TYPES_CONFIGURATOR":{"severity":"HIGH"},"votingPeriod":{"severity":"HIGH"},"votingDelay":{"severity":"HIGH"},"proposalThreshold":{"severity":"HIGH"},"token":{"severity":"HIGH"},"admin":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"timelock":{"severity":"HIGH"},"manager":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: None
      fieldMeta:
+        {"withdrawPaused":{"severity":"MEDIUM"},"depositPaused":{"severity":"MEDIUM"},"circleCaller":{"severity":"HIGH"},"l2USDC":{"severity":"HIGH"},"l1USDC":{"severity":"HIGH"},"router":{"severity":"HIGH"},"messenger":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: Counterpart to the L1GatewayRouter contract.
      fieldMeta:
+        {"ethGateway":{"severity":"HIGH"},"defaultERC20Gateway":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description: ETH is pre-minted to this contract in the genesis block and released on Scroll whenever corresponding deposits are made on Ethereum.
      fieldMeta:
+        {"paused":{"severity":"MEDIUM"},"messageQueue":{"severity":"HIGH"},"owner":{"severity":"HIGH"},"$admin":{"severity":"HIGH"},"counterpart":{"severity":"HIGH"}}
    }
```
...
```diff
    ...
    +++ description:  Contract that uses controllers to manage minters for USDC on Scroll.
      fieldMeta:
+        {"getMinterManager":{"severity":"HIGH"},"owner":{"severity":"HIGH"}}
    }
```
...
Generated with discovered.json: 0xd1ff2e3a6252be24ac68f522c463f88faa26582f

# Diff at Fri, 18 Sep 2026 10:25:02 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@e2faf827d006bceee5fb0904599ba325066c7674 block: 1787147569
- current timestamp: 1789723805
...
Generated with discovered.json: 0xfc9ce05e1b96e9f0d1bf72940121a74559d37e19
