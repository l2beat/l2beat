...
Upgrade to op-contracts/v4.1.0 (Upgrade 14: Isthmus L1 Contracts + MT-Cannon, https://gov.optimism.io/t/upgrade-proposal-14-isthmus-l1-contracts-mt-cannon/9796) executed across two transactions (2026-03-16 and 2026-03-25). Upgrading from op-contracts/v2.2.0. Major changes: MIPS replaced with MIPS64 (→ 1.8.0), switching from single-threaded MIPS32 to multi-threaded 64-bit Cannon ("MT-Cannon") with thread-safe memory and 8-byte preimage reads. OptimismPortal2 (3.13.0 → 5.0.0) adds ETH_LOCKBOX feature flag for future interop and EIP-7702 EOA detection. SystemConfig (2.4.0 → 3.7.0) exposes guardian, paused, superchainConfig, l2ChainId, operatorFee params; maximumGasLimit increased 200M → 500M. Also upgraded: L1CrossDomainMessenger (2.5.0 → 2.10.0), L1StandardBridge (2.2.2 → 2.7.0), L1ERC721Bridge (2.3.1 → 2.8.0), DisputeGameFactory (1.0.1 → 1.2.0). New instances: AnchorStateRegistry (→ 3.5.0), DelayedWETH (→ 1.5.0, delay 302400s/3.5d), PermissionedDisputeGame (→ 1.7.0). Also includes the shared OpFoundation signer rotation.
