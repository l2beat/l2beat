Generated with discovered.json: 0x162d282e28cf349d2fc1d0c512355b5b3246cd4f

# Diff at Wed, 23 Sep 2026 05:47:55 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@2e9174e8edc4a3b646f958a1d6e0d4360abec40d block: 1777043298
...
```diff
    contract DelayedWETH (eth:0x6d4A27F3eb196E4d039a5EE5B510Cc4C89b7eA9a) [opstack/DelayedWETH] {
...
```diff
    contract AddressManager (eth:0xEa4165C5CDCA155779803A113d8391b741bA5228) [opstack/AddressManager] {
...
```diff
    contract L1CrossDomainMessenger (eth:0xf80be9f7a74ab776b69d3F0dC5C08c39b3A0bA19) [opstack/L1CrossDomainMessenger] {
...
Generated with discovered.json: 0xdd0f70c5ed7030460b39907a698e5365b82e8350

# Diff at Mon, 21 Sep 2026 11:24:03 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@231e4a5828ee5ff5a863f7b80215466ca39a5b1e block: 1777043298
- current timestamp: 1777043298
...
Generated with discovered.json: 0x6e4584176248292c07d35ef408939d7b0253b279

# Diff at Fri, 18 Sep 2026 10:24:51 GMT:

- author: sekuba (<29250140+sekuba@users.noreply.github.com>)
- comparing to: main@e2faf827d006bceee5fb0904599ba325066c7674 block: 1777043298
- current timestamp: 1777043298
...
```diff
    contract SuperchainConfig (eth:0x2cF123e0328AF3Ae81c331A029a5f837Dc2231ad) [opstack/SuperchainConfigFake] {
...
```diff
    contract AnchorStateRegistry (eth:0x807c2aF3Ae99a20576B00b58327B594D47f837FB) [opstack/AnchorStateRegistry] {
...
Generated with discovered.json: 0xdd8a947649c38611a5bf416c074cf4a3f0b3d5b2
...
- comparing to: main@488d190650457a1fba9b18a83f14a17ab8b2c84c block: 1777043298
- current timestamp: 1777043298
...
```diff
    contract L1StandardBridge (eth:0x2b3F201543adF73160bA42E1a5b7750024F30420) [N/A] {
    +++ description: None
      sourceHashes.1:
-        "0xc8d5e683491427e9263869a1ea42ae938fe1ea1c6a8cdccd7a86a13140b3c22d"
+        "0x8eab89b4d5a9a0888682f25207bebb442fbd7a6ad04e42815ed5522c3f65d9ba"
    }
```
...
```diff
    contract OptimismPortal2 (eth:0x59625d1FE0Eeb8114a4d13c863978F39b3471781) [N/A] {
    +++ description: None
      sourceHashes.1:
-        "0x77a056b7a1c5ddb643a0168b7b9436ee0c2d7edf89da2f7c4bf674100a9fc4b5"
+        "0xafd41f4a1c64be415e5ba2c2cd7b118365cf1f6184b597d4b15bffeb41fee019"
    }
```
...
```diff
    contract L1ERC721Bridge (eth:0xC2C908F3226d9082130D8e48378CD2eFb08B521D) [opstack/L1ERC721Bridge] {
...
```diff
    contract OptimismMintableERC20Factory (eth:0xF02012065Ef6121a2A59EA0C590f42803Cf101EA) [opstack/OptimismMintableERC20Factory] {
...
Generated with discovered.json: 0x6cb24490bca50633950bb46dcee9d4e7af92abb0

# Diff at Tue, 05 May 2026 10:22:28 GMT:

- author: Mateusz Radomski (<radomski.main@protonmail.com>)
- comparing to: main@b6437082b3ea8fb0d97f4474b1c3452a1ce271b0 block: 1777043298
- current timestamp: 1777043298
...
```diff
    ...
    +++ description: None
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
```

```diff
    contract SuperchainConfig (eth:0x2cF123e0328AF3Ae81c331A029a5f837Dc2231ad) {
    +++ description: This is NOT the shared SuperchainConfig contract of the OP stack Superchain but rather a local fork. It manages the `PAUSED_SLOT`, a boolean value indicating whether the local chain is paused, and `GUARDIAN_SLOT`, the address of the guardian which can pause and unpause the system.
      deployerAddress:
+        "eth:0x9e4d3d601b76a68aD306D9863698F0894cD052bd"
    }
```
...
```diff
    ...
    +++ description: None
      deployerAddress:
+        "eth:0x7fa31d53Aaa649Bbf08ba2fEa0aE5B6b71cd2ccd"
    }
```
...
```diff
    contract DelayedWETH (eth:0x6d4A27F3eb196E4d039a5EE5B510Cc4C89b7eA9a) {
    +++ description: Contract designed to hold the bonded ETH for each game. It is designed as a wrapper around WETH to allow an owner to function as a backstop if a game would incorrectly distribute funds.
      deployerAddress:
...
```diff
    contract AnchorStateRegistry (eth:0x807c2aF3Ae99a20576B00b58327B594D47f837FB) {
    +++ description: Contains the latest confirmed state root that can be used as a starting point in a dispute game.
      deployerAddress:
...
```diff
    contract L1ERC721Bridge (eth:0xC2C908F3226d9082130D8e48378CD2eFb08B521D) {
    +++ description: Used to bridge ERC-721 tokens from host chain to this chain.
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
...
```diff
    contract PermissionedDisputeGame (eth:0xC7Ab73aE577650565cE7337b09704570C3e379b7) {
    +++ description: Same as FaultDisputeGame, but only two permissioned addresses are designated as proposer and challenger.
      deployerAddress:
+        "eth:0xBA8Dc81a29c4d9bE2630d1158B60179A50D2A95c"
    }
```

```diff
    contract SystemConfig (eth:0xC975862927797812371A9Fb631f83F8f5e2240D5) {
    +++ description: Contains configuration parameters such as the Sequencer address, gas limit on this chain and the unsafe block signer address.
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
...
```diff
    contract ProxyAdmin (eth:0xDa5C416025D5eED8440667C5E80A7c35c2F56220) {
    +++ description: None
      deployerAddress:
+        "eth:0x9e4d3d601b76a68aD306D9863698F0894cD052bd"
    }
...
```diff
    contract AddressManager (eth:0xEa4165C5CDCA155779803A113d8391b741bA5228) {
    +++ description: Legacy contract used to manage a mapping of string names to addresses. Modern OP stack uses a different standard proxy system instead, but this contract is still necessary for backwards compatibility with several older contracts.
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
...
```diff
    contract OptimismMintableERC20Factory (eth:0xF02012065Ef6121a2A59EA0C590f42803Cf101EA) {
    +++ description: A helper contract that generates OptimismMintableERC20 contracts on the network it's deployed to. OptimismMintableERC20 is a standard extension of the base ERC20 token contract designed to allow the L1StandardBridge contracts to mint and burn tokens. This makes it possible to use an OptimismMintableERC20 as this chain's representation of a token on the host chain, or vice-versa.
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
...
```diff
    contract L1CrossDomainMessenger (eth:0xf80be9f7a74ab776b69d3F0dC5C08c39b3A0bA19) {
    +++ description: Sends messages from host chain to this chain, and relays messages back onto host chain. In the event that a message sent from host chain to this chain is rejected for exceeding this chain's epoch gas limit, it can be resubmitted via this contract's replay function.
      deployerAddress:
+        "eth:0xD23d1DAF772BDc525842Ebf2B379A305d89DC914"
    }
...
- author: vincfurc (<vincfurc@users.noreply.github.com>)
- comparing to: main@55ab80636f1e0c000e757a7a146f11035a19e9c0 block: 1765382149
...
Following changes come from updates made to the config file,
or/and contracts becoming verified, not from differences found during
discovery. Values are for block 1765382149 (main branch discovery), not current.
...
Generated with discovered.json: 0x9ca5aa3cad8eebe4a35386b4d8dc08248b6bea69

# Diff at Fri, 12 Dec 2025 16:19:58 GMT:

- author: vincfurc (<vincfurc@users.noreply.github.com>)
- comparing to: main@d2235ef7f5c90cb4a3a617fea7d52a655dc22fa1 block: 1765382149
- current timestamp: 1765382149
...
+        "eth:0x2cF123e0328AF3Ae81c331A029a5f837Dc2231ad"
    }
```

```diff
-   Status: DELETED
    contract PermissionedDisputeGame (eth:0x3E2986901f7149b61C2F11603b165BAf9D33d531)
    +++ description: Same as FaultDisputeGame, but only two permissioned addresses are designated as proposer and challenger.
```
...
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0xd81f43eDBCAcb4c29a9bA38a13Ee5d79278270cC"]]
      values.$pastUpgrades.8:
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0xB443Da3e07052204A02d630a8933dAc05a0d6fB4"]]
      values.$upgradeCount:
-        7
+        9
      values.guardian:
...
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0x5e40B9231B86984b5150507046e354dbFbeD3d9e"]]
      values.$upgradeCount:
-        2
+        4
      values.config:
...
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0xd81f43eDBCAcb4c29a9bA38a13Ee5d79278270cC"]]
      values.$pastUpgrades.2:
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0x7b465370BB7A333f99edd19599EB7Fb1c2D3F8D2"]]
      values.$upgradeCount:
-        1
+        3
      values.superchainConfig:
...
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0xd81f43eDBCAcb4c29a9bA38a13Ee5d79278270cC"]]
      values.$pastUpgrades.8:
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0x7aE1d3BD877a4C5CA257404ce26BE93A02C98013"]]
      values.$upgradeCount:
-        7
+        9
      values.superchainConfig:
...
+        "eth:0x560aFA9cF6B39D8C83938C77036E80807a56Da16"
      values.owner:
-        "eth:0x4a4962275DF8C60a80d3a25faEc5AA7De116A746"
+        "eth:0x3c54Be307E15258E51d42E585831bD7E04BBc03b"
      values.unsafeBlockSigner:
-        "eth:0x41c4FAE5E80B9a622d8968bcd3EBbcf1F93b30Db"
+        "eth:0xEf8452Bfe63c7a370504bcEDCF19D07f2dC6412f"
    }
```
...
+        "eth:0xC7Ab73aE577650565cE7337b09704570C3e379b7"
      values.owner:
-        "eth:0x4a4962275DF8C60a80d3a25faEc5AA7De116A746"
+        "eth:0x3c54Be307E15258E51d42E585831bD7E04BBc03b"
    }
...
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0xd81f43eDBCAcb4c29a9bA38a13Ee5d79278270cC"]]
      values.$pastUpgrades.9:
+        ["2025-12-08T17:26:23.000Z","0x4946776c2d4568490d0280564f07c06afb9080b3e69af8d1db72321c6148cf55",["eth:0x5D5a095665886119693F0B41d8DFeE78da033e8B"]]
      values.$upgradeCount:
-        8
+        10
      values.superchainConfig:
...
# Diff at Fri, 05 Dec 2025 11:23:18 GMT:

- author: vincfurc (<vincfurc@users.noreply.github.com>)
- comparing to: main@1edf3e71cea32596658a3ea017cea9df6408b77c block: 1759480887
- current timestamp: 1764933721
...
Generated with discovered.json: 0x54aca2f628af57851dbdcce6f341ad94030e36ce

# Diff at Tue, 04 Nov 2025 11:33:36 GMT:

- author: Mateusz Radomski (<radomski.main@protonmail.com>)
- comparing to: main@9ff7b62a511791b99f61b604fb6b56e4ea223bb0 block: 1759480887
...
Following changes come from updates made to the config file,
or/and contracts becoming verified, not from differences found during
discovery. Values are for block 1759480887 (main branch discovery), not current.
...
Generated with discovered.json: 0xa415d49d9f9f0e3edc74d34e01987b13476f8ee3
