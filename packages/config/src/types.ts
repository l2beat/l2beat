...
/**
 * Shape of the committed stake-distribution.json files generated by
...
export const ProjectStakeDistributionSchema = v
  ...
  .check(
    (d) =>
      d.totalStake > 0 &&
      (d.validatorCount === undefined ||
        (Number.isInteger(d.validatorCount) && d.validatorCount > 0)) &&
      (d.entities === undefined || d.entities.every((e) => e.stake >= 0)),
    'totalStake must be positive, validatorCount a positive integer and entity stakes non-negative',
  )
...
export type ProjectInclusionDelayChartEntityStake = NonNullable<
  ProjectInclusionDelayChartStakeDistribution['entities']
>[number]
