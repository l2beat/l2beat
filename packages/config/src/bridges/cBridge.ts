...
export const cBridge: Bridge = {
  ...
  technology: {
    ...
    principleOfOperation: {
      ...
      description:
        'Celer cBridge is a hybrid solution able to work in two modes: Token Bridge and Liquidity Network.\nIn Token Bridge mode user locks tokens in the escrow account and mints tokens on destination network. When bridging back to Ethereum tokens are burned on source chain and transferred back to the receiver on Ethereum.\n In Liquidity Network mode user deposits tokens into the pool on the source chain and withdraws a matching number of tokens from the pool on the destination chain based on a bridge rate generated by the StableSwap pricing curve.',
      references: [
        {
