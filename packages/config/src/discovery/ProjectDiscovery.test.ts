...
describe(ProjectDiscovery.name, () => {
  ...
  describe(ProjectDiscovery.prototype.getEoaActors.name, () => {
    ...
    it('should handle EOAs with direct act permissions', () => {
      const signerRegistry = ChainSpecificAddress(
        'eth:0x2222222222222222222222222222222222222222',
      )
      const configReaderDirectAct = mockObject<ConfigReader>({
        readConfig: (projectName: string) => mockConfig(projectName),
        readDiscoveryWithReferences: () => [
          {
            ...discoveredJsonStub,
      ...
      const discoveryDirectAct = new ProjectDiscovery(
        'DirectActProject',
        configReaderDirectAct,
      )

      const result = discoveryDirectAct.getEoaActors()

      expect(result.raw).toHaveLength(2)
      expect(result.linkable).toHaveLength(2)
      expect(result.grouped).toHaveLength(1)
      expect(result.grouped[0].name).toEqual('DirectSigner1 and DirectSigner2')
      expect(result.grouped[0].accounts).toHaveLength(2)
      expect(result.grouped[0].description).toEqual(
        '* Can act through SignerRegistry\n  * sign messages',
      )
    })
