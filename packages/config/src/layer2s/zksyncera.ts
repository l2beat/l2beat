...
export const zksyncera: Layer2 = {
  ...
  upgradesAndGovernance:
    'Currently, the Matter Labs multisig (4/8, compare below zkSync Era multisig) is permissioned to instantly change all contracts (including the diamond and its facets) and roles (including the *Governor* role). The *Governor* role that resolves to the multisig is the highest permissioned role defined in the system.\n\n*Governor:* Can access all `AdminFacet` functions. Additionally inherits permissions to functions for the *Admin* role. Can freeze all freezable Facets (currently `ExecutorFacet`, `MailboxFacet`) and can upgrade the bridges. \n\n*Validator:* Proposes batches from L2 into the `ValidatorTimelock`, from where they can be proven and finally executed (through the `ExecutorFacet` of the diamond) after a predefined delay (currently 21h). This allows for freezing the L2 chain within the delay if any suspicious activity was detected. Can be set by the *Admin* or *Governor*. \n\n*Verifier:* Verifies the zk proofs that were provided by the Validator. Can be changed by calling `ExecuteUpgrade()` on the `AdminFacet` from the *Governor* role. \n\n*Admin:* Currently **not set**. Will be able to make non-critical changes like setting the *Validator*. Will be the role of the multisig when higher permissions are restricted to the *Security Council*.\n\n*Security Council:* Currently **not set**. Will have the *Governor* role (2/2) with the Matter Labs multisig to limit its power.\n\nA `Governance` smart contract is currently in place between the Matter Labs multisig and the diamond. It has the *Governor* role and is fully controlled by the multisig. It includes logic for planning upgrades with parameters like transparency and/or a delay and has a slot for the potential *Security Council* role. Currently the delay is optional and not used by the multisig. The optional transparency may be used in the future to hide emergency upgrades by the *Security Council* or delay transparent (thus auditable) governance upgrades.',
  permissions: [
