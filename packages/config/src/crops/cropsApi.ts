...
import { CROP_ATTESTATIONS, getCurrentCropAttestation } from './attestations'
import {
  CROP_KEYS,
  CropSentimentSchema,
  CropStatusSchema,
...
// The CROPS API contract, shared by the crops-api generator that writes the
// files and the frontend page that documents them. The validators are the
// contract: the OpenAPI document is built from them, the types are inferred
// from them, and every generated file is parsed with them in tests. Objects
// are strict so a field config adds but the spec does not know fails the test
// instead of leaking. `meta({ description })` reaches the OpenAPI document.
