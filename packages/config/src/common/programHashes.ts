...
const OUTER_BOOTLOADER = (version: string) => ({
  ...
  description:
    'Top-level Cairo program executed by SHARP. The GPS verifier pins its compiled felts directly through a CairoBootloaderProgram contract and reconstructs them as public memory. This catalog identifier is the standard Cairo Pedersen program hash derived from the rebuilt executable; the GPS contract stores the felts rather than this hash.',
  proverSystemProject: ProjectId('stwo'),
})
...
const SUPPORTED_SIMPLE_BOOTLOADERS = (version: string) => ({
  ...
  description:
    'Pedersen commitment to the ordered list of accepted simple-bootloader executable hashes. The list covers the simple and no-builtin-simulation bootloaders under the supported program-hash encodings. A malicious accepted bootloader can falsify unpacked task outputs and register application facts not backed by the intended child executions.',
  proverSystemProject: ProjectId('stwo'),
})
...
const SUPPORTED_CAIRO_VERIFIERS = (version: string) => ({
  ...
  description:
    'Pedersen commitment to the ordered allowlist of Cairo programs that may recursively verify and unpack nested SHARP proofs. The commitment alone does not reveal the list length. It is reproducible only when its ordered preimage and every verifier program behind the listed hashes are published and reproducible.',
  proverSystemProject: ProjectId('stwo'),
})
...
const SHARP_AGG = (prover: string) => ({
  ...
  description:
    'Aggregates proofs of correct execution for several consecutive transaction ranges generated by Starknet OS. The onchain identifier is the domain-separated `H("AGGREGATOR", rawAggregatorProgramHash)` value emitted by the applicative bootloader, so reproduction must cover both the raw program and this transformation.',
...
Note: \`cargo prove vkey --elf <path-to-elf-file>\` prints a different SP1 vkey representation for this program, not the \`hash_bytes()\` program hash used here.
  ...
  '2549868507195840500193135872505150687001846773665388230794631345999578394351':
    {
      ...
      description:
        'Pedersen commitment to the ordered recursive Cairo-verifier allowlist. Verification requires the exact ordered preimage and a reproducible source/compiler/hash recipe for every listed program.',
      verificationStatus: 'notVerified',
    },
  ...
  '2989448937132554463006794084002640731746256535824175616421304143852713734169':
    {
      ...
      description:
        'Pedersen commitment to the ordered recursive Cairo-verifier allowlist. Its preimage is published; verification still requires a reproducible source/compiler/hash recipe for every listed program.',
      programUrl:
  ...
  '160268921359133235574810995023520895391777547407923205700393332203861498631':
    {
      title:
        'Simple bootloader data commitment StarkWare_GpsStatementVerifier_2024_10',
      description:
        'Direct Poseidon commitment used by this legacy verifier to pin its simple-bootloader data. Unlike modern versions, this value is not a Pedersen commitment to an ordered list of supported executable hashes. A malicious committed bootloader can falsify task outputs and register application facts not backed by the intended child execution.',
      proverSystemProject: ProjectId('stwo'),
      programUrl:
