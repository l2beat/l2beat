...
describe('socket', () => {
  it('plugs should be up to date with the discovery output', async () => {
    ...
    assert(
      ...
      'There are new plugs in the socket discovery output. Go to packages/config and run yarn update-socket, then handle the crawl results from the script.',
    )
    try {
      expect(plugs.every((plug) => addresses.includes(plug))).toEqual(true)
    } catch (error) {
      console.error(
        'The crawl results from the socket script are outdated. Go to packages/config and run yarn update-socket, then handle the crawl results from the script.',
      )
      throw error
    }
  })
