...
| `l2b add-shape <project> <addressOrName>` | Look up the contract in `discovered.json`, create the template (default `<project>/<contractName>`) if missing, and add the shape. For upgradeable contracts, use the address(es) from `$implementation`, not the proxy address; for diamonds, use all implementation/facet addresses. The address(es) written to `shapes.json` must be the same ones whose flattened sources produced the stored hash. Use `--template <path>` and `--name <fileName>` to add multiple impls to one template under different keys. Pass `--description "..."` and `--ignore-relatives "field1,field2,..."` to populate the template in one shot (insert-only; errors out if either field is already set so you never silently clobber existing edits). |
...
├── shapes.json               generated by add-shape (flattened-source hashes; for proxies store implementation address(es), not the proxy address)
...
- **For proxy-backed templates, `shapes.json` must point at implementations.** When creating a shape for an EIP-1967 / UUPS / transparent proxy, copy the address from the discovered entry's `$implementation` field into `shapes.json`; do not use the proxy address. Template matching uses the implementation hash, and maintenance tools fetch source from the address stored in `shapes.json`, so the stored address must point to the same implementation source that produced the hash. For diamonds, store all implementation/facet addresses that contributed to the combined hash.
