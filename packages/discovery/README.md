...
Each pair is represented as a `[key, value]` array.
...
The input array typically comes from the `to_entries` filter.
If multiple entries have the same key, the last one wins.
