...
import { type EthereumAddress, Retries } from '@l2beat/shared-pure'
...
export class SourcifyClient implements SourceProvider {
  ...
  private async call(address: string): Promise<unknown | null> {
    const fields = ['abi', 'sources', 'compilation', 'creationBytecode']
