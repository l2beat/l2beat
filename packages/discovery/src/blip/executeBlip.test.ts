...
describe(executeBlip.name, () => {
  // Contract value tests (assuming contract values can be used in comparisons)
  ...
  it('supports complex nested logical expressions', () => {
    expect(
      executeBlip({}, ['and', ['not', false], ['!!', 'test'], ['=', 1, 1]]),
    ).toEqual(true)
  ...
  it('handles multiple and nested expressions', () => {
    expect(executeBlip({}, ['and', true, ['!!', 1], ['not', false]])).toEqual(
      true,
    )
    expect(executeBlip({}, ['and', true, ['!!', 0], ['not', false]])).toEqual(
      false,
    )
  })
  ...
  it('compares multiple values for equality', () => {
    ...
    expect(executeBlip({}, ['=', 'a', 'a', 'b'])).toEqual(false)
  })
  ...
  it('converts values to boolean', () => {
    ...
    expect(executeBlip({}, ['!!', 0])).toEqual(false)
    expect(executeBlip({}, ['!!', 1])).toEqual(true)
    expect(executeBlip({}, ['!!', ''])).toEqual(false)
    expect(executeBlip({}, ['!!', 'hello'])).toEqual(true)
  })
