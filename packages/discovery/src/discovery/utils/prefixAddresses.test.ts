...
describe(prefixAddresses.name, () => {
  ...
  it('does nothing to primitives', () => {
    expect(prefixAddresses(CHAIN, 'foo')).toStrictEqual('foo')
    expect(prefixAddresses(CHAIN, 1)).toStrictEqual(1)
    expect(prefixAddresses(CHAIN, 0)).toStrictEqual(0)
    expect(prefixAddresses(CHAIN, -0)).toStrictEqual(-0)
    expect(prefixAddresses(CHAIN, -1)).toStrictEqual(-1)
    expect(prefixAddresses(CHAIN, false)).toStrictEqual(false)
    expect(prefixAddresses(CHAIN, true)).toStrictEqual(true)
  })

  it('does nothing to arrays with primitives', () => {
    const arr = ['foo', 1, 0, -0, -1, false, true]
    expect(prefixAddresses(CHAIN, arr)).toStrictEqual(arr)
  })

  it('does nothing to objects with primitives', () => {
    const obj = { k1: 'foo', k2: 1, k3: 0, k4: -0, k5: -1, k6: false, k7: true }
    expect(prefixAddresses(CHAIN, obj)).toStrictEqual(obj)
  })

  it('prefixes a string address', () => {
    const address = '0x33D66941465ac776C38096cb1bc496C673aE7390'
    expect(prefixAddresses(CHAIN, address)).toStrictEqual(
  ...
  it('prefixes array of addresses', () => {
    ...
    expect(prefixAddresses(CHAIN, addresses)).toStrictEqual([
  ...
  it('prefixes addresses in objects values and keys', () => {
    ...
    expect(prefixAddresses(CHAIN, obj)).toStrictEqual({
      'eth:0x07D5c2f0eC7f791F1BB0760C7BaC21Eef10a0956':
        'eth:0x33D66941465ac776C38096cb1bc496C673aE7390',
    })
  })
