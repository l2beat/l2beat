...
export function printDiscoverySummary(
  ...
  if (skippedDueToCap.length > 0) {
    logger.warn(
      chalk.yellowBright(
        `  WARNING: ${skippedDueToCap.length} address(es) skipped due to maxAddresses cap (${maxAddresses}).`,
      ),
    )
    logger.warn(
      chalk.yellowBright(
        '  Discovery is incomplete. Bump maxAddresses in config.jsonc and re-run to inspect.',
      ),
    )
  }
