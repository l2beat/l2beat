...
describe('diff', () => {
  ...
  it('same arrays with primitives', () => {
    expect(diff([1, 2, 3], [1, 2, 3])).toStrictEqual([])
    expect(diff([1n, 2n, 3n], [1n, 2n, 3n])).toStrictEqual([])
    expect(diff(['A', 'B', 'C'], ['A', 'B', 'C'])).toStrictEqual([])
    expect(diff([true, false, true], [true, false, true])).toStrictEqual([])
    expect(
      diff([null, undefined, null], [null, undefined, null]),
    ).toStrictEqual([])
  })
  ...
  it('same arrays with objects', () => {
    ...
    expect(diff(left, right)).toStrictEqual([])
  })
  ...
  it('removal/creation/change in array on primitives', () => {
    expect(diff([1, 2, 3], [1, 2, 3, 4])).toStrictEqual([
      { kind: 'create', path: [3], rhs: 4 },
    ])

    expect(diff([1, 2, 3, 4], [1, 2, 3])).toStrictEqual([
      { kind: 'remove', path: [3], lhs: 4 },
    ])

    expect(diff([1, 3], [1, 2, 3])).toStrictEqual([
      { kind: 'create', path: [1], rhs: 2 },
    ])

    expect(diff([1, 2, 3], [1, 3])).toStrictEqual([
      { kind: 'remove', path: [1], lhs: 2 },
    ])

    expect(diff([1, 2, 3], [1, 42, 3])).toStrictEqual([
  ...
  it('removal/creation/change in array on objects', () => {
    ...
    expect(diff(right, left)).toStrictEqual([
  ...
  it('arrays perform LCS diff on primitives', () => {
    expect(diff(['D', 'A', 'B', 'C'], ['A', 'B', 'C'])).toStrictEqual([
      { kind: 'remove', path: [0], lhs: 'D' },
    ])

    expect(diff(['A', 'B', 'C'], ['D', 'A', 'B', 'C'])).toStrictEqual([
      { kind: 'create', path: [0], rhs: 'D' },
    ])

    expect(diff(['A', 'C'], ['A', 'B', 'C'])).toStrictEqual([
      { kind: 'create', path: [1], rhs: 'B' },
    ])

    expect(diff(['A', 'B', 'C', 'D'], ['B', 'D'])).toStrictEqual([
      { kind: 'remove', path: [0], lhs: 'A' },
      { kind: 'remove', path: [2], lhs: 'C' },
    ])

    expect(diff(['B'], ['A', 'B', 'C', 'D'])).toStrictEqual([
      { kind: 'create', path: [0], rhs: 'A' },
      { kind: 'create', path: [2], rhs: 'C' },
      { kind: 'create', path: [3], rhs: 'D' },
    ])

    expect(diff(['A', 'B', 'D'], ['B', 'C', 'D'])).toStrictEqual([
      { kind: 'remove', path: [0], lhs: 'A' },
      { kind: 'create', path: [1], rhs: 'C' },
    ])

    expect(diff(['X', 'Y', 'Z'], ['A', 'B', 'C'])).toStrictEqual([
  ...
  it('arrays perform LCS diff on objects', () => {
    ...
    ).toStrictEqual([{ kind: 'remove', path: [0], lhs: { id: 'D' } }])
    ...
    ).toStrictEqual([{ kind: 'create', path: [0], rhs: { id: 'D' } }])

    expect(
      diff([{ id: 'A' }, { id: 'C' }], [{ id: 'A' }, { id: 'B' }, { id: 'C' }]),
    ).toStrictEqual([{ kind: 'create', path: [1], rhs: { id: 'B' } }])
  ...
  describe('edge cases', () => {
    it('creates and removes inside arrays of BigInt', () => {
      expect(diff([1n, 2n, 3n], [0n, 1n, 3n, 4n])).toStrictEqual([
    ...
    it('deep change inside nested arrays', () => {
      expect(diff([[[1, 2]]], [[[1, 3]]])).toStrictEqual([
        { kind: 'change', path: [0, 0, 1], lhs: 2, rhs: 3 },
      ])
    })

    it('change of Date instances inside an array', () => {
      expect(diff([new Date(1)], [new Date(2)])).toStrictEqual([
    ...
    it('reorder of primitives is expressed as remove + create', () => {
      expect(diff(['A', 'B'], ['B', 'A'])).toStrictEqual([
        { kind: 'remove', path: [0], lhs: 'A' },
    ...
    it('handles identical objects that share reference', () => {
      ...
      expect(diff([shared, shared], [shared, shared])).toStrictEqual([])
    })

    it('handles sparse arrays (holes treated as undefined)', () => {
      expect(diff([, 1], [undefined, 1])).toStrictEqual([])
    })

    it('object to array transition inside object property', () => {
      expect(diff({ list: { a: 1 } }, { list: [{ a: 1 }] })).toStrictEqual([
...
describe('microdiff tests', () => {
  it('new raw value', () => {
    expect(diff({ test: true }, { test: true, test2: true })).toStrictEqual([
      { kind: 'create', path: ['test2'], rhs: true },
    ])
  })
  it('change raw value', () => {
    expect(diff({ test: true }, { test: false })).toStrictEqual([
      { kind: 'change', path: ['test'], rhs: false, lhs: true },
    ])
  })
  it('remove raw value', () => {
    expect(diff({ test: true, test2: true }, { test: true })).toStrictEqual([
      { kind: 'remove', path: ['test2'], lhs: true },
    ])
  })

  it('replace object with null', () => {
    expect(diff({ object: { test: true } }, { object: null })).toStrictEqual([
  ...
  it('equal null protype objects', () => {
    expect(diff(Object.create(null), Object.create(null))).toStrictEqual([])
  })
  ...
  it('unequal null protype objects', () => {
    ...
    obj2.test = true
    expect(diff(obj1, obj2)).toStrictEqual([
  ...
  it('Handles unequal number classes', () => {
    ...
    expect(diff({ number: lhs }, { number: rhs })).toStrictEqual([
      { kind: 'change', path: ['number'], rhs, lhs },
    ])
  })

  it('Handles equal dates', () => {
    expect(diff({ date: new Date(1) }, { date: new Date(1) })).toStrictEqual([])
  })

  it('Handles unequal dates', () => {
    expect(diff({ date: new Date(1) }, { date: new Date(2) })).toStrictEqual([
      { path: ['date'], kind: 'change', rhs: new Date(2), lhs: new Date(1) },
    ])
  })

  it('Handles value being a date and the other not', () => {
    expect(diff({ date: new Date(1) }, { date: 'not date' })).toStrictEqual([
      { path: ['date'], kind: 'change', rhs: 'not date', lhs: new Date(1) },
    ])

    expect(diff({ date: 'not date' }, { date: new Date(1) })).toStrictEqual([
      { path: ['date'], kind: 'change', rhs: new Date(1), lhs: 'not date' },
    ])
  })

  it('new NaN value in object', () => {
    expect(diff({}, { testNaN: Number.NaN })).toStrictEqual([
      { kind: 'create', path: ['testNaN'], rhs: Number.NaN },
    ])
  })

  it('change NaN value in object', () => {
    expect(diff({ testNaN: Number.NaN }, { testNaN: 0 })).toStrictEqual([
  ...
  it('do not change NaN value in object', () => {
    expect(
      diff({ testNaN: Number.NaN }, { testNaN: Number.NaN }),
    ).toStrictEqual([])
  })
  ...
  it('new NaN value in array', () => {
    expect(diff([], [Number.NaN])).toStrictEqual([
      { kind: 'create', path: [0], rhs: Number.NaN },
    ])
  })

  it('change NaN value in object', () => {
    expect(diff([Number.NaN], [0])).toStrictEqual([
      { kind: 'change', path: [0], rhs: 0, lhs: Number.NaN },
    ])
  })

  it('do not change NaN value in array', () => {
    expect(diff([Number.NaN], [Number.NaN])).toStrictEqual([])
  })

  it('Handles equal regex', () => {
    expect(diff({ regex: /a/ }, { regex: /a/ })).toStrictEqual([])
  })

  it('Handles unequal regex', () => {
    expect(diff({ regex: /a/ }, { regex: /b/ })).toStrictEqual([
      { kind: 'change', path: ['regex'], rhs: /b/, lhs: /a/ },
    ])
  })

  it('top level array & array diff', () => {
    expect(diff(['test', 'testing'], ['test'])).toStrictEqual([
      { kind: 'remove', path: [1], lhs: 'testing' },
    ])
  })

  it('nested array', () => {
    expect(diff(['test', ['test']], ['test', ['test', 'test2']])).toStrictEqual(
  ...
  it('array to object', () => {
    expect(diff({ data: [] }, { data: { val: 'test' } })).toStrictEqual([
