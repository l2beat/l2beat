...
export async function getYieldFiMinters(
  provider: IProvider,
  ...
  const mintersAndRedeemers = await extractAccessControlMembers(
    provider,
    chainSpecificAdmin,
    field,
    abi,
  ...
  for (const orRole of ['BRIDGE_ROLE', 'LOCKBOX_ROLE', 'YIELD_ROLE']) {
    const members = await extractAccessControlMembers(
      provider,
      chainSpecificAdmin,
      field,
      abi,
...
async function extractAccessControlMembers(
  provider: IProvider,
  ...
  try {
    const handler = new AccessControlHandler(field, definition, abi)
