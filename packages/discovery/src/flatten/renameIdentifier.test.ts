...
describe(renameIdentifiers.name, () => {
  ...
  it('renames a contract declaration name', () => {
    ...
    expect(result).toBe('contract Base_1 { function f() public {} }')
  })
  ...
  it('renames a contract declaration name and body self-references', () => {
    ...
    expect(result).toBe('contract Foo_1 { function f(Foo_1 x) public {} }')
  })
  ...
  it('renames a struct declaration name', () => {
    ...
    expect(result).toBe('struct MyData_1 { uint256 value; }')
  })
  ...
  it('renames an enum declaration name', () => {
    ...
    expect(result).toBe('enum Status_1 { Active, Inactive }')
  })
  ...
  it('renames a function call in a state variable initializer (length mismatch)', () => {
    ...
    expect(renameIdentifiers(src, [{ from: 'test', to: 'X' }])).toBe(
      'contract Foo { uint256 x = X(); }',
    )
    expect(renameIdentifiers(src, [{ from: 'test', to: 'XXXXXX' }])).toBe(
