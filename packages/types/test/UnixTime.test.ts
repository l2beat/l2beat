...
describe(UnixTime.name, () => {
  ...
  describe(UnixTime.isNumberUnixTimestamp.name, () => {
    it('correct timestamp', () => {
      const timestamp = 1661873521
      expect(UnixTime.isNumberUnixTimestamp(timestamp)).toEqual(true)
    })
    it('too small number', () => {
      const timestamp = new Date('3000-01-01T00:00:00.001Z').getTime()
      expect(UnixTime.isNumberUnixTimestamp(timestamp)).toEqual(false)
    })
    it('too big number', () => {
