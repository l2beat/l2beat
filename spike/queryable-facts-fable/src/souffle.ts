...
export function runSouffle(opts: {
  ...
  const run = spawnToFiles(SOUFFLE, args, opts.out)
  const ms = performance.now() - t0
  if (run.error)
    throw new SouffleError(
      `could not run '${SOUFFLE}': ${run.error.message} (set $SOUFFLE)`,
      '',
      '',
    )
  if (run.status !== 0)
  ...
  return { ms, command: [SOUFFLE, ...args].join(' ') }
}

/**
 * Runs a command with stdin/stdout/stderr bound to files instead of pipes, so it also works inside
 * sandboxes that forbid socket pairs. The outputs stay in `dir` next to what the command wrote.
 */
 ...
 * Soufflé writes every output relation with its annotations; one output keeps exactly what the
 * question needs.
 */
