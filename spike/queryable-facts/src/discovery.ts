...
//   dPermission(To, Direct, Permission, From, Role, Via) discovery's own permission model, for comparison
...
export interface DiscoveredPermission {
  ...
  description?: string
  condition?: string
  via?: Array<{ address: string; condition?: string }>
}
...
export interface DiscoveredJson {
  ...
  permissions?: Record<
    string,
    {
      receivedPermissions?: DiscoveredPermission[]
      directlyReceivedPermissions?: DiscoveredPermission[]
    }
  >
...
export function discoveryFacts(project: Project): Facts {
  ...
  for (const [to, perms] of Object.entries(discovered.permissions ?? {})) {
    for (const p of perms.directlyReceivedPermissions ?? [])
      facts.add(
        'dPermission',
        to,
        1,
        ...
        p.from,
        p.role ?? '',
        (p.via ?? []).map((v) => v.address).join(' > '),
      )
    for (const p of perms.receivedPermissions ?? [])
      facts.add(
        'dPermission',
        to,
        0,
