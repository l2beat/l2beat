...
export function Atom({ text }: { text: string }) {
  ...
  return (
    <span className="atom">
      ...
      {match.cols.map((c, i) => (
        <span key={i}>
          <Cell value={c} type={columns[i]?.type} />
...
function Snippet({ text }: { text: string }) {
  ...
  if (!match || !match.isFact) return null
  const ref = index.findFactRow(match.relation, match.cols)
  const row = ref ? index.row(ref) : undefined
